...
- **D22 (build, W0) — only a reserve WITH a `floorEffect` counts as floored.** `VitalsMixin.getConditionBand` added a severity point for every biological reserve at zero — so `wind` and `alcohol-tolerance`, seeded empty by design, would have made every fresh body read *unwell* (severity 2). One line in `Vitals.ts` (`r.floorEffect !== null`) — the semantics `Reserve.floorEffect` already declares ("named effect when current hits the floor"; null = no floor). Build-4 keeps that line unchanged, so the merge is clean. Decided by the nearest pattern (the field's own doc) and the requirements' "a fresh body reads in good flesh and nothing more".
- **D23 (build, W1) — the farming acts keep their felt cost as the authored figure; the base converts.** `FieldStepOptions.cost` (the `%`-points a fresh body pays) stays, and `FieldWorkController.engageAct` turns it into watts through `giver.wattsForFeltCost(cost, durationS)` — a body-independent inverse (it reads `exertion.baseSustainableW`, never the body's wind) — so the plough still costs exactly `14 / draught` and a conditioned body still feels it as less. Why not constant watts as in mining: a field act's duration is an abstraction (4 s to lime a field), and the constant that reproduces 4 % over 4 s is 1800 W — pinning a number like that per act would be pretending it was physics. Mining and the smelt keep the plan's constants (967/750/900/425) and inherit the "harder ground costs more" consequence.
- **D24 (build, W1) — `char` declares no `effortW`.** The plan gave the burn 200 W; but the burn is three game-DAYS on the `attention` slot, and "duration at a sustainable pace" would max the collier's wind from a single burn. The activity's own doc says it is watched, not worked; not work, by its own declaration. The mill is `620 W` (plan: 550): above a fresh body's overload threshold (0.7 × 840 = 588 W) and below the threshold a few points of lean later (~57), so the sack trains a new miller and stops training one who has outgrown it — the drive's step 6/6b in one number.
- **D25 (build, W4) — the orange is a par line at the GENERAL STORE, not the cash-and-carry; D13's first half is re-planned.** `distribution` → `trade-farming` is a **pack cycle**: farming already depends on distribution because its hand consigns onto the distributor's counter, and the distributor is the hub every producing trade points at, which must not point back (its own README says so; `PackApi` refuses the cycle at install). And the cycle is telling the truth about the economy (lens 6): the Wharfside counter already GETS the farm's citrus — the `farms` brain spawns crates into the farm's stock and the hand carries them to the cash-and-carry and consigns them at an ask — so a par line there would be the distributor conjuring fruit beside a real supply. The par line a person can count on goes on the terminus general store (which depends on farming and distribution already, sells the seeds, and is "where the grocer has not fragmented off yet" — settlement doctrine), price 1. The barbell line stays on the cash-and-carry (smithing does not depend on distribution; no cycle). The drive buys the orange at the general store.
- **D26 (build, W5) — the barbell is priced on the SMITHY'S OWN SLATE, not the cash-and-carry; the gym archetype ships without its `pace` slot.** Two gates, both right. (a) `distribution` → `trade-smithing` is a second pack cycle (smithing → cooking → distribution), so D13's other half re-plans the same way as D25: the bar is a line on the Hearthworks `smithy-menu` (`order barbell`, 14 — beside the smith's hammer at 12), which is the maker's own counter and the honest one; `distribution/package.json` is untouched. (b) `lint:capabilities` treats an archetype need as a REQUIREMENT, not a consumer as the plan assumed, and refused `pace` as "required and nothing offers it". The gate is stating the plan's own risk #5 (no sustained device ships) as a rule: the slot lands with the first `pace` device, one line; the gym header records the seam. The archetype is `load` + `mat` today.
- **D27 (build, W7 — what the drive found) — the ladder AFFORDS `climb`.** The plan's reachability table called `climb` "universal": it is not. `Mobile` contributes `go`/`sneak`/`run`/`teleport`; nothing anywhere contributed `climb` (or `swim`, or `fly`), so the verb was `unknown-verb` at the winze head — the first link failing closed. The instrument affords the verb: `Ladder.commandContributions` puts `climb` in the `environment` bucket, exactly as the watering can affords `water`. Swim and fly stay unafforded (not this build's water or sky).
- **D28 (build, W7) — a FORCED command's dispatch-response is stamped `forced: true`, and the wire harness skips it.** The auto-`sense` on arrival fires its own envelope INSIDE `run`'s, before `run`'s own, so a correlator that takes "the next dispatch-response" read the sense's `[]` as the run's outcome and handed `run`'s real envelope — `pace-broken` and all — to the NEXT command. Every movement verb on the wire has been mis-correlated this way; the first run of this drive saw the run "never break" and then break one command late. One field on `DispatchResponseEnvelope` (`CommandGiver.emitDispatchResponse` reads the innermost Command frame's `forced`), one `continue` in `Session.receive`. The client is unaffected (it does not correlate).
- **D29 (build, W7) — three pre-existing defects the drive surfaced, fixed where the fix is a row:** (a) the bakery's business listed only the ROOM as an operating location, and a sale settles against the counter's path, so nobody had ever bought a loaf there — the grain-chain drive's `buy loaf` accepted "it sells or it says why" (`bakery-business.yaml` now lists the bread-counter); (b) `buy orange` at the general store handed over a packet of orange SEED (the packet answers to the bare crop name, farming's convention, and `buy` takes the first match) — the fruit's line now sits above the seeds; (c) a positional `type: number` arg reaches its validators as the raw string, so `mustBeNumber` refuses every honest `lift 60` — dropped from `lift.yaml` (the controller parses); ⚠ `draw <amount>` carries the same validator and has never been driven — banking's to look at.
- **D30 (review, 2026-09-19) — every limit is soft; work never collapses a body.** The reviewer asked whether the instrumentation walls a player who overexerts. Inventory: the run breaks to a walk, the climb narrates, `lift` refuses a load, `canExert` refuses a step at the spent line — none interrupts anything but the next shift; walking is free. The one wall in the game is pre-existing `collapse` (endurance 0 → `requiresConscious` on ~56 verbs incl. `go`/`eat`), which mining could already reach and which this build's climb (12 %/ladder, no pace check) and combat exchange (1.6 % each) could newly walk a body into. Fix: `exert` clamps the endurance debit at `exertion.exhaustionFloorPct` — an exertion spends you to the spent line and never past it; a body the limp took under the line is left there. Collapse stays the acute conditions' and the wound's. (`Exerting.test.ts` pins it: 14 → 10 on a ladder, a hundred exchanges at 10 cost nothing, 4 stays 4.)
- **D31 (review, 2026-09-19 — W8) — feedback: the body as words, at the moment it changes.** The reviewer asked how a player monitors this in real time without hit points. Three surfaces, all bands: (1) `bodyState` — a subscribable field on `Creature` (breath/hunger/thirst/build as words) joins `SHELF_SUBSCRIPTION` and a `BODY` row joins the shelf catalogue AND the default shelf (it started answering; the shelf shows the breath word and adds hunger/thirst only when they say something); (2) `ExertingMixin.noteBodyState()` after every `exert` and every metabolism reconcile (a new `Metabolic.onMetabolismReconciled` `@hook` terminal the mixin overrides) pokes `notifyDurableSubject` only when a band turned over; (3) a `self.body` cue on a breath crossing — down: *"You're winded."* / *"You're spent."*; up: one *"You've got your breath back."* for the whole climb; spent → winded says nothing; the first read of a session is silent. New dial `exertion.freshPct` (70). The prompt (`{{ body }}` in `prompt.format`) was considered and left: the shelf and the cue cover it, and a prompt word is a template a player can add later. Drive: 23/23 — the figure reads `fresh` on a fresh body and `winded` after the run breaks; the cue lands on the exit that crossed, before the next run breaks.
...
**✅ Done** (`build(nutrition-fitness W0)`). Notes: (1) D22 above — the Vitals floor guard was a real shipped-defect-in-waiting the plan missed; (2) lean's relaxation is the exact exponential `(lean − seed)(e^{−Δt/τ} − 1)`, not the linear slope — the reconcile collapses a long gap's remainder into ONE step, and a linear step longer than τ overshot the seed in the first test run; (3) `getMass()` reads the stocks RAW (no reconcile) because it sits on the `wouldExceedCeiling` hot path and the reconcile itself reads mass; (4) `bodyMassIndex()` returns `null` (band `healthy`) for a body with no species stature rather than NaN; (5) the `dial()` function is duplicated per module (Freshness/Metabolic/Creature — the module-private shape the plan chose; a shared helper would be a free function).
...
**✅ Done** (`build(nutrition-fitness W1)`). Notes: (1) D23/D24 above; (2) the census is 28 `engageStep` sites, not 25 — forestry landed three (`fell` 700 / crosscut 500 / coppice 400 W) while this was being planned, and merging master first is what found them; (3) the mining act test's premise moved — endurance is paid AT COMPLETION now, not up front, and its `Miner` fixture composes `ExertingMixin` (a `TestActor` is an Idea with no body, so `ceilingW` narrows `isTangible` before reading mass); (4) the combat exchange's exertion shows up in `CombatLogic.hooks`' lifesteal pin (−1.6 % each) and is asserted there rather than in a separate `Character.exchange` test; (5) the climb's rest line landed here in `exertTraverse` rather than W2 — same function, one edit.
...
**✅ Done** (`build(nutrition-fitness W4)`). Notes: (1) D25 above — the plan's counter placement hit a pack cycle the moment the dependency was added, and the cycle was right; (2) the ten produce rows carry real mg/kg figures (orange/lemon 530, lime 290, grapefruit 310, mint 310, cranberry 130, cherry 70, grape 30; olive gets `fibre` + fat, juniper only water/fibre); (3) the wholemeal loaf is priced 3 at the bakery counter (white 4, lean 2); (4) the label-derivation test lives in the baking pack over the milling pack's real flour/bran rows, so it checks the shipped numbers and not a stub; (5) `pnpm install` was run after the package edit and again after reverting it.
...
**✅ Done** (`build(nutrition-fitness W5)`). Notes: (1) D26 above; (2) a new controller needs its seed row (`platform/idea/cmd/device/LiftController.yaml`, `data: {}`) or `controller-seeds.integrity` fails — the plan's file list missed it; (3) `lift` reads the strain ceiling through the encumbrance gauge, so a TIRED body finds the bar too heavy before it finds itself too tired (the endurance margin shrinks capacity first) — honest, and the test pins the order; (4) `lint:capabilities --list` reports `load` offered by trade-smithing and consumed by the archetype, the view and the controller.
...
**✅ Done** — `design/harm-survey` merged to master (`45235900d`) while W7 was being driven, so the gate opened mid-build: master merged (four conflicts — `mixin.ts` two predicates side by side, `AssessController` imports, its test's two new describe blocks, the pharma slate's two new sections), `depositWorkHeat` forwards to `absorbHeatLoad` under `isThermalRegulation`, `canSustainPace` is false while `capacity('locomotion') !== 'full'` (fitness is not a bandage), `Exerting.heat.test.ts` (27 kJ per 600 W-minute; a 1-clo body sheds slower and spends more water over the same shift; the impaired leg). ⚠ Two things the test taught: a test class minted per call under a repeated `_mixinName` collides in the registry on the second test (module-scope classes), and `StuffApi.clearAll()` takes the WorldClockRegistry with it (the heat-load test resets only the clock). Drive step 7 runs on the wire: the shift makes H sweat. ⓘ The sweat CUE cannot tell coated from bare over one set — even the general store's 3 kg wool gambeson is ~0.2 clo body-weighted — so the hydration half is proven at unit level; and cold air does NOT stop the sweating (the internal load sheds through the cold branch too — you sweat shovelling snow), which corrects the requirements' "cold air produces neither".
...
**✅ Done** — see the drive record below. W8 (review) added two checkpoints: the `BODY` figure over the wire and the winded cue at the crossing — 23/23.
...
| **the climb** | `climb` (exists, never reachable) | ~~universal~~ **the ladder** — `Ladder.commandContributions.environment` (D27: nothing contributed the verb; the drive found `unknown-verb`) | `Ladder` class + `winze-ladder` row + the `down`/`up` exits with `media: [vertical]` | the rejection pack installs the rows; `Ladder` is instanceable under `/platform/thing/` | the enablement walk finds the ladder in the room's contents |
...
`packages/wire/tests/nutrition-fitness.dirty.wire.test.ts`, run with `WIRE_BOOT=1` on a freshly dropped `saxonberg_build3` (2026-09-18, fourteen runs — the last eight on the merged tree with W6, on `WIRE_PORT=2014` after a sibling's owned boot reaped the world on 2012 mid-run). **Final: 22 passed, 0 failed** (step 7 included). The first run was **8/22**. What the runs found, in order:

| run | found | what it was |
|---|---|---|
| 1 | the run "never broke" in step 4, then broke one command LATE in step 10 | **harness**: the auto-`sense` on arrival fires its own dispatch-response inside `run`'s; the harness correlates by order and read the sense's `[]` as the run's outcome. Every movement verb on the wire has been mis-correlated this way. → `DispatchResponseEnvelope.forced` + `Session.receive` skips it (D28) |
| 1 | `climb down` → `unknown-verb` at the winze head | **reachability, link 2**: nothing contributed `climb` (or swim, or fly) — `Mobile` affords `go`/`run`/`sneak` only. → the ladder affords it (D27) |
| 1 | `order barbell` → `insufficient-heat` | the forge was cold; an anvil order needs the forge lit and pumped (the cooking drive ignites the oven first). → drive |
| 1 | L's bare `look` → `empty-result[target]` | L's focus was still H, who had left the room. → drive (`inventory`) |
| 1 | `eat orange` → `validator-failed[target]` | **world**: `buy orange` at the general store handed over a packet of orange SEED — the packet answers to the bare crop name and `buy` takes the first match. → the fruit's line above the seeds (D29b) |
| 1 | `buy wholemeal` → `insufficient-funds` with 500 in the bank | **world, pre-existing**: the bakery's business listed only the ROOM as an operating location; a sale settles against the counter's path. Nobody had ever bought a loaf there — the grain-chain drive's `buy loaf` accepted "it sells or it says why". → `bakery-business.yaml` lists the bread-counter (D29a) |
| 2 | `look wholemeal` timed out with no dispatch-response | four identical loaves on the counter: a bare `look wholemeal` raised a WHICH prompt the harness cannot answer. → `look me:i:wholemeal` |
| 2 | `lift 60` → `validator-failed[load]` | **kernel, pre-existing**: a positional `type: number` arg reaches its validators as the raw string; `mustBeNumber` refuses every honest number. ⚠ `draw <amount>` carries the same validator and has never been driven. → dropped from `lift.yaml`; the controller parses (D29c) |
| 2 | scurvy did not clear two game-minutes after the orange | the pool absorbs at 2 %-points a game-minute: 30 points take a quarter of a game-hour. → drive idles 12 game-min |
| 3 | `lift 60` → `too-heavy` | **honest**: H had just run until winded (step 4) and the strain ceiling shrinks with endurance; a winded body cannot get 60 off the floor |
| 4 | `lie` → `no-posture-slot` on the smithy floor | **world finding**: the forge floor authors no posture slot and most rooms have no floor at all, so resting by lying down is impossible almost everywhere. → L runs step 4 instead; H lifts fresh |
| 5 | L's climb had no rest line | L's runs in step 10 had happened with the season dial still up (one run = wind 60). → the season ends before anyone runs again |
| 5 | L's mass moved 0.03 kg | **honest**: a sprint sits just over a fresh body's overload line; a tape would not notice. → tolerance 0.2 kg |
| 7–13 (W6, step 7) | `clone` access-denied at the smithy; `lift` after `wear` → `engagement-conflict`; no sweat cue at the forge, none on the lane | the founder holds no title on Hearthworks' parcel → H **buys the general store's wool gambeson**; donning is an engagement → awaited; **the forge room's wet-bulb sits over the ceiling where sweat cannot shed heat** (the model is honest: no cue) → the shift moves to the lane; and the cue is a debounced EPISODE flag that had already fired during step 6's sets at the forge (ambient + load) and re-arms only in the comfort band — collected from step 6 on. `wear gambeson` ok + dressing step, then `remove gambeson` → `not-worn`: an equipment-verb finding, tolerated |

Every checkpoint asserts a sentence, a note kind or a projected field. The seven the plan said only the drive could prove all passed on the wire: `lift` afforded from the floor, `climb down` binding, a `config` change moving a season inside one session, two bodies diverging on `look` from one start, `buy orange` → `eat` clearing scurvy end to end, `survey` naming the gym in a room with a bar on the floor, and the two loaves' labels.

**The browser walk (2026-09-19, a real Chromium on the real client, `browser-h2` / `browser-l` / the founder — the chrome-devtools profile was held by a sibling session, so Playwright drove it):** the shelf's `BODY` read *fresh* on arrival and flipped to *winded* on the exit that crossed, with *"You're winded."* in the feed and *"You're winded — you drop to a walk."* on the next run; `assess` said *of a healthy weight*; `order barbell` at the lit forge, `drop`, `survey` → *"a gym: yes — has load (an iron barbell), mat (a smithy workbench)"*; `lift 60` while winded → *"You can't get that off the floor."* (the strain ceiling shrinking with endurance — honest); three sets of `lift 40` at the season dials, each cheaper than the last as wind rose; `competence` → *wind — expert*; `look browser-h2` from a second character → the card reads *"In good flesh, hard."* Two things the browser found that the wire could not: (1) the shelf's **`SKILL` row read `alcohol-tolerance · untrained` on a brand-new character** — `practisingCompetence` folds to the digest's first row and the conditioning overlay sorted ahead of nothing; a stock is not *practised* — the practising figure now reads the transcript fold alone; (2) **`competence` on a fresh character opened with `alcohol-tolerance — untrained`** — a zero in a costume; an untrained stock is no longer listed, the floor implicit as it is for the fold. Pre-existing, noted: the forced auto-`sense` echoes as `here> sense` in the feed on every arrival; `look human` with two humans in the room prompts *which target?* with yourself in the list.

**Findings for other trades / builds (not fixed here):** `wear`→`remove` of a bought gambeson answers `not-worn` after an ok dressing step (equipment); `draw <amount>` (banking) is refused by its own `mustBeNumber`; the grain-chain drive's `buy loaf` assertion should now be `expectOk` (its session has no money — it needs the bank step); almost no room has a floor with a `lie` slot (residences/furnishing); the wire harness had no way to tell a forced command's envelope from a typed one until now (`docs/testing.md` should say so).
