...
**Files added:**
- `lib/character/HaulingCreature.ts` — `HaulingCreature = MountableMixin(PostRegistrationMixin(Character))`. The cart-pulling capability comes from `Character` (which composes `HaulerMixin` — see below); this class adds the **rideable** slot (`Mountable`) so the worked example (a horse you ride while it hauls) works. Draft-animal seeds derive from it. **As-built note** (review): `HaulerMixin` lands on **`Character`** (not `Avatar` + this class as first planned) per MR feedback — Character is the agency layer (every PC + NPC-character), still off the broad `Creature` base (the encumbrance doc's *"compose the mixin, not the class tree"* rule; a frog/corpse never hauls).
...
**Decisions resolved:** `HaulerMixin` composes on the **`Character`** agency layer (every PC + NPC-character can pull a cart), **not** the `Creature` base (don't broaden every sprite/fish/corpse with a hauling ref). `HaulingCreature` inherits it and adds `Mountable`. (First built on `Avatar` + an explicit wrapper on `HaulingCreature`; moved to `Character` per MR review — simpler, and the agency layer is the right scope.) `Character` is a `Creature`, so the `LoadBearing` prerequisite for the draft term still holds. *Linkdead* (non-guest body lingering for reconnect) keeps the transient ref in memory (harmless; cart waits in the room; a restart drops the Pattern-B ref) — the testable cleanup cases (cart destruct, real logout) ride `onDestruct` + the self-heal getter. Flag below.
...
- `obj/api/LocomotionLogic.ts` — in `canTraverseExit`, after `checkEnablement` passes, append a haulage check (private `checkHaulage(actor, exit, mode)`) gated on `MixinApi.isHauling(actor) && actor.isHitched()`. **The hauler is the actual traverser (`actor`)** — for a ridden move that's the host (the horse) whose own self-powered traverse is gated (see flag #6: the originally-planned `findConveyanceHost`-from-rider resolution was dead code — `exit.canTraverse` rejects passthrough modes before `checkHaulage` runs). This is the **veto layer explicitly allowed to read encumbrance / draft / strain ceiling**; walk has no `enablementMixin` so the gates can't live in `checkEnablementScope`. Two sub-gates (read off the actor's cart):
...
6. **Ridden gate is host-as-actor, not findConveyanceHost (discovered at build).** The plan assumed `canTraverseExit(rider, ride, …)` would reach `checkHaulage` and resolve the host via `findConveyanceHost`. It does NOT: `exit.canTraverse` rejects passthrough modes (`ride`/`drive`) at the media gate (`allowsMode` returns false for a `medium: null` mode) *before* `checkHaulage` runs — a **pre-existing locomotion limitation** (mounted exit-traversal via `ride <dir>` is itself media-blocked, independent of haulage). So `checkHaulage` was simplified to `hauler = actor`: the gate that fires for a ridden move is the **host's own self-powered traverse** (the horse is the actor + the hauler), which the integration test verifies directly (`canTraverseExit(steed, stairs, walk) → terrain`). The tow works regardless (host.traverse tows). **Wiring the ridden gate into the `ride`/`drive` dispatch is blocked on fixing the pre-existing passthrough-exit limitation** — out of scope for haulage; flagged for a locomotion follow-up. The draft term + tow + host-side gate are all correct and tested at the substrate.
