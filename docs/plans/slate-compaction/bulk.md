...
1. **Two `Left` items were false on arrival.** bulkable-slate named
   *"`Container`+`Bulkable`"* — it shipped as `CraftVessel`
   ...
   remains is that `Crate` (`CirculatingMixin(PopulatesMixin(ContainerMixin(…)))`,
