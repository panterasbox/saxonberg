...
| ⚠⚠ …and run 20 still hauled the keepnet. **The scorer was inert behind every scope seed**: `reachable` / `inventory` / `peers` stamp each candidate 100 and the keyword narrow kept `max(100, keyword)` — so every `scope:word` query (the default arg shape of most verbs) tied at 100 and `top` fell to pool order. The 60-tier could not help; nothing read it | `filterByKeywords` (kernel MQL) | the narrow carries the keyword's own score; in situ test: a keepnet in hand, a net on the floor, `reachable:net` → the net. 3,517 command/MQL/controller tests unchanged. This is the class behind `look edge`, `drop sturgeon` (the first of several by pool order), `haul net` — every substring collision the drives keep finding was really this |
...
| run 22: the keepnet checkpoint's instrument — no wizard holds the bank, so there is no eval there at all | the drive | the read is diegetic: `release` refuses a dead fish and lets a live one go, and its inventory scope means the bank's litter cannot confuse it |
| ⚠⚠ run 23: **a fish a minute dead in the hand was released alive.** The drain's last act opens a `dying` record and cancels itself; the record's clock was stamped by the FIRST RECONCILE after `beginDying`, so a body nobody read never moved toward death — the clock that "does not freeze on linkdead" froze on being unobserved, and `release`'s `isDead()` put the fish back into the record as a live count | `Vitals.beginDying` (kernel mortality) | `beginDying` stamps `tickedAt` now; two kernel tests that read 1,000 s past a 600 s window and expected *dying* read inside it. `ReleaseController` asks the reconciling read (`getDyingRemainingSec`; the expiry's death fires async, a beat later) and treats a window with nothing left as dead (`e9ebffd41`) |
| the snap step lost a coin-toss (eight casts, no fighter strong enough to part the line before it landed) | the drive | the wizard makes the full fighter the water's commonest fish for that step, as the sturgeon step does |

**Run 24 — 21/21, `EXIT 0`, 28 min** (2026-09-22, owned world on 2013, fresh DB): the seventeen original steps and the four of step 17 — the leger over a surface shoal vs the float; the big hook over the crabs never takes a crab and the plain hook does; the spoon left to lie takes nothing and worked takes a hunter; a fish a minute in a laid keepnet is alive and the water takes it back, a minute in the hand and the water will not.
