...
| W6 run 3 | 15/15 | — |
| **the browser** (2026-09-18, `pnpm dev` + Chrome, one character, the whole script) | every step walks | **(a)** the wire's step 8 had PASSED on a refusal: `shore` with nothing in hand answers *"You have no timber to set"* and `/timber/i` matched it — in the browser the set had to be picked up (`get set`), and `shore` is an ENGAGED act whose set is consumed at COMPLETION (walk out mid-engagement and 40 kg stays in your hands). The wire now takes the set, waits for the completion, and asserts *"timber takes the weight"*. **(b)** after a HARD kill (`dev:clean`) the oak clearing's sixteen trunks were gone while the ride's earlier trunk, every stand count, the sapling and the cut panels survived: a player's goods ride the OWNER's estate, which a live player writes only on the residency cadence or at logout; a graceful stop (what every wire run had) flushes it, a kill loses the window. Every felling/cross-cut now also captures the FELLER; verified with a kill seconds after a felling — both trunks, the logs and the ash key in hand came back. **(c)** prose: *"4 logs come off the crown"* / *"5 lengths in it yet"* were digits (now words); the chronicle line was lowercase and unpunctuated beside *"Arrived at the fuel yard."* (now *"Planted an oak sapling in the oak clearing."*). Also seen: the card surface is LIVE — the bole card's mass and *"Five lengths in it yet"* updated as it was cross-cut; the SKILL widget read *silviculture · untrained* after the first cut; `get log` while over the carry ceiling says *"a log doesn't budge"* (the LOG is 4 kg — the refusal blames the object, not the load: encumbrance's prose, for the sweep); a killed-then-rebooted transient yard re-mints its axe and billhook beside the ones a player carried off (the known transient-props class). |
| final tree, again (after the feller capture; on `WIRE_PORT=2013` — a sibling worktree was running its own wire worlds on 2012 and the harness's "world up" poll twice caught THEIRS: old yard prose, no `harvest`, 15/15 failures that were not mine) | **15/15** | the acorn is now the landed predicate for a felling (it is the last thing minted; the logs land first). |
