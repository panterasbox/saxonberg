...
| **trade-farming** | `lime`, `lemon`, `orange`, `grapefruit`, `mint`, `cherry`, `olive`, `cranberry` (food materials with tags = category), plus `crop/seed/plant/pot/bed` rows **moved** from generic-objects (`git mv`, paths `/trade/farming/thing/{crop,seed,plant,pot,bed}/…`) | `crate`, `basket` (Container things) | eight crates (the fruit rows are `Tangible` globs — `/platform/thing/Provision`'s shape — inside a crate; `Circulating` on the crate) | — (produce is an input; `press` is hospitality's) |
...
| `…/material/spirit/lime.yaml` | `trade-farming/…/idea/material/lime.yaml` | the daiquiri recipe (`category: lime` → `kind: item` produce + `press`) |
...
| `generic-objects/…/thing/{crop,seed,plant,pot,bed}/*` | `trade-farming/…/thing/{crop,seed,plant,pot,bed}/*` | `world-seed` dorm/Hinkley rows that populate pots/beds (`grep -rn "/stuff/thing/\(crop\|seed\|plant\|pot\|bed\)/" packages/content packages/server/src`), `generic-objects/pack.yaml` claims (drop the five), `trade-farming/pack.yaml` claims them under `/trade/farming` |
