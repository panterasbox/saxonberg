# Ch 1 (re-record) — "The whole machine, end to end"

> **Re-record sequence** (see [rerecord-outline.md](./rerecord-outline.md)) — the
> **summary chapter.** The one video someone can watch *alone* and come away with a basic,
> end-to-end understanding of the whole thing. The later chapters (Ch 2–7) are this one
> exploded — each takes one beat here to higher resolution. **This chapter is canonical
> for Ch 1; the outline's "How a decision moves" section is superseded by this file.**
> **Its own video in the playlist.**
>
> **Scope: the main chapters ONLY (Ch 2–7).** No appendix material (the land / compute /
> ratification-config are optional depth, out of the summary).
> **Register: plain explainer** — clear and accessible, no sales pitch. It states *what
> things are*, not their full justifications — **with one exception: the opening carries
> the basic *why this exists.***
>
> **Opens on:** why anyone would build this at all. **Closes:** the dial — any community
> can run it, from one operator to a full republic, at its own pace.
...
**consumer** — gets its own **chamber**, and none can outvote the other
...
record no one can quietly rewrite. Not "trust us, the vote was honest." Check it yourself.
You never have to take anyone's word for it.

**[close — the dial]**
And none of it is all-or-nothing. This isn't one game — it's a machine any community can
pick up, a Discord, a guild, a stream, and run at whatever setting it trusts itself with:
a **dial** that turns from *one operator deciding everything* to *a full republic where the
...
- **Close — hero, corner→wide.** The full assembled machine, then a **dial** on it turning
  *operator → republic*; ghosted other communities each adopting the machine and setting
  their own dial. Hand-forward to the deep dives.

## Notes

- **The one justification kept** is Beat 0 (*why this exists*) — per the brief, Ch 1 states
  what things *are*, not their reasoning, **except** the basic why-we're-building-it. Beat 0
  is the compressed Ch 2 payload (self-governance lacks the participation engine; the game
  is it); the fuller argument is Ch 2.
- **Scope = Ch 2–7 only.** No appendix (land / compute / ratification-config). Deliberately.
- **Close now matches Ch 7 and FIXES the parked inconsistency.** The old Ch 1 close ("control
  dilutes… the structure is built to give itself away") oversold an *automatic* handover;
  Ch 7 established it's a **dial the founder chooses to turn — nobody forced.** The Close is
  rewritten to the dial framing ("opens up as far and as fast as that founder chooses… the
  structure doesn't make you give it away, it just makes it safe to"). The
  reconcile-with-Ch1 [OPEN] in `rerecord-ch3.md`'s Close note is now resolved.
- **Coverage map** (each beat → its deep dive): 0 → Ch 2 · ① game/code-split → Ch 2 (+ sets
...
- **⚠ VOCABULARY — lead with the spine, not the gloss.** The formal terms are **labor /
  ...
  friendly game gloss. The audience reads political economy — don't talk down by using only
  the gloss. Introduced formally in ①, referenced in ④, and the firewall lands on
  **"capital"** by name. (A prior draft dropped the spine entirely — regression, fixed
  2026-07-16.) Matches the locked vocab in `rerecord-outline.md` + `candidate-beats.md`.
- **⚠ It's ONE system — do NOT frame it as "two systems / two halves" (that's backstory,
  keep it OUT of the video).** The *lineage* (the user's own mental model / inspiration):
  closest to a **corporate model**, but with power **not concentrated in a board** —
  distributed across the three constituent forces (labor / capital / consumer); plus a
  **public/private-property** notion that lends it some **state** character. That genealogy
  is useful for *understanding* the design, but it does **not** belong in the narration. The
  video just *describes the single system*: a game, three chambers, three branches, a
  record, a dial. Two earlier drafts erred — (a) hand-waving "like any economy," and (b)
  over-correcting into "borrows from two systems / market half + state half"; **both cut
  2026-07-16.** Don't reach for a framing device; describe the thing.
- **① leads with GAME *and* GOVERNMENT** (per the brief — this is a government video, so
  don't open on "it's a game" alone). The **EVE Online** nod grounds "a real government in a
  game" in the famous case (player corporations/alliances/economy; CCP put an economist on
...
- **Length:** was pinned at ~5 min in the old single-video plan; the playlist relaxes it,
  but this one *should* stay tight — it's the summary, and its job is a clean end-to-end
  pass, not depth.
