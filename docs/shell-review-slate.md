...
| B + L + E | `write` requires explicit `-c` / `-s` (no `workspace.tree` fallback — content / source diverge by purpose). Content writes accept `--class` and `--hydrator` opts (defaults `/lib/stuff/Idea` + `/lib/persistence/PersistentHydrator`; pass `--hydrator=` to omit). Source writes ignore both. Body delivery via greedy positional OR structured-form (`assembleFromStructured`) — both populate `model.body` as string; verified the structured path passes coercion. Sidecar deferral note already in WriteController docstring. Also caught and migrated the `--mql` opt to `type: object` (missed in thread D's sweep since write was being touched separately). | ✓ 947fcd8 |
