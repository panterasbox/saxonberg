...
| K | `cmd/teleport.yaml` destination defaults to `$focus` (was undefined → controller errored). New `_resolveDestinationContainer` rule: Container focus → use as-is (room, chest, vessel interior, avatar inventory), Containable-only → walk to its environment ("next to" semantics). Container-wins precedence covers Avatars and Vessels — focus on bob, teleport sword → into bob's inventory; focus on wagon → into the wagon. Override case still uses `to <where>` preposition. | ✓ pending commit |
