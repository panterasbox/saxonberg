...
| G | `autoLookOnArrival` retired as a standalone exported function in `Mobile.ts`. Promoted to a method on the `Mobile` interface — internal callers (`traverse` / `teleport`) and `GotoController` all use `mover.autoLookOnArrival()` instead of importing. | ✓ pending commit |
