...
| write-mirror-fix (B followup) | Course-correction: `write` should only require explicit `-c` / `-s` under `mirror` mode, not always — single-tree modes can use the `workspace.tree` setting like the read verbs. WriteController.execute() relaxed: mutual-exclusivity check + mirror-only requirement, then `giver.pickTree(model)`. Docstring + `shell-workspace.md` write row + B+L+E slate row updated. | ✓ 0511b90 |
| visible-no-verbs (C/F/G followup) | `Visible` drops `look.yaml` from its `environment`/`inventory`/`peers` contributions — the prior shape granted `look` to a non-Perceiver actor *because there happened to be a visible thing nearby*, inverting the actor-capability contract (the throne-contributes-`sit` shape is correct because `sit` is verb-against-target; `look` is perceiver-side, target-resolved through scope). Visible is now pure target shape: description state, keywords, no verbs. Perceiver is the sole source of `look`. Docs + Character composition comments updated. | ✓ 47a8807 |
| write-data-payload (B+L+E followup) | Reviewer caught that `write -c` was stashing `{ body: model.body }` into the template's `data` field — but the body IS the data, not a sub-key on it. Split the payload: `data` (`type: struct`, payload-only) for content writes, `body` (`type: string`, payload-only) for source writes. Controller passes `model.data` straight through to `TemplateApi.saveTemplate`. Validation hook: classes opt in by exporting `static dataSchema: Record<string, unknown>` (JSON Schema fragment); when present, the controller validates `model.data` via the matcher's existing `validateAgainstJsonSchema` helper (now exported). The matcher's struct-validation path is sync, but `loadClassByPath` is async, so the schema check moved to the (async) controller — same machinery, different phase. Widened `FieldValue` to include `Record<string, unknown>` so struct values land cleanly on typed model interfaces. | ✓ pending commit |
