...
| `write` | `<path>`, `--mql`, `-c`, `-s`, `--class`, `--hydrator`, `payload.body` | Honors `workspace.tree` like the read verbs when it picks a single tree (`content` / `source`). Under `mirror` mode the setting doesn't pick — writing the same body to both is nonsensical, so an explicit `-c` or `-s` is required. The two flags are mutually exclusive when both supplied. **Body is payload-only** — declared under the yaml's `payload:` block, populated through `CommandApi.assembleFromStructured({ verb: 'write', fields: { path, body, ... } })`, never typeable through the shell. See [command-spec.md § payload](./command-spec.md) for the broader pattern. Content writes accept `--class` (default `/lib/stuff/Idea`) and `--hydrator` (default `/lib/persistence/PersistentHydrator`; pass `--hydrator=` to omit). Source writes ignore both. |
