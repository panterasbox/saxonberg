...
| `write` | `<path>`, `--mql`, `-c`, `-s`, `--class`, `--hydrator`, `payload.body` | Requires explicit `-c` or `-s` — `workspace.tree` is NOT consulted as a fallback (content / source diverge by purpose). **Body is payload-only** — declared under the yaml's `payload:` block, populated through `CommandApi.assembleFromStructured({ verb: 'write', fields: { path, body, ... } })`, never typeable through the shell. See [command-spec.md § payload](./command-spec.md) for the broader pattern. Content writes accept `--class` (default `/lib/stuff/Idea`) and `--hydrator` (default `/lib/persistence/PersistentHydrator`; pass `--hydrator=` to omit). Source writes ignore both. |
