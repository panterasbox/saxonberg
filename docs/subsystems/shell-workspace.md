...
| `write` | `<path>`, `<body>`, `--mql`, `-c`, `-s` | Body lands on `model.body` via either path: text input (greedy positional, fine for short bodies) or `CommandApi.assembleFromStructured({ verb: 'write', fields: { path, body } })` (the structured form, no tokenization — what a code-editor UI uses). The arg is named `body` rather than `content` so the `--content` flag stays free for tree selection. |
