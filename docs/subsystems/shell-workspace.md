...
| `write` | `<path>`, `<body>`, `--mql`, `-c`, `-s`, `--class`, `--hydrator` | Requires explicit `-c` or `-s` — `workspace.tree` is NOT consulted as a fallback (content / source diverge by purpose). Body lands on `model.body` via either path: text input (greedy positional, fine for short bodies) or `CommandApi.assembleFromStructured({ verb: 'write', fields: { path, body } })` (the structured form, no tokenization — what a code-editor UI uses). The arg is named `body` rather than `content` so the `--content` flag stays free for tree selection. Content writes accept `--class` (default `/lib/stuff/Idea`) and `--hydrator` (default `/lib/persistence/PersistentHydrator`; pass `--hydrator=` to omit). Source writes ignore both. |
