...
| **trade-brewing** · **trade-winemaking** · **trade-bottling** (STUB trades — libations) | platform, base-library, distribution | default | `/trade/<x>` → group `<x>` (PM-owned): everything **downstream of production and nothing of production** — materials tagged by category (`carbonated` on sparkling and the sodas; bottling's `ice` = frozen water with `density`/`meltingPoint`/`latentHeatOfFusion`), the vessel presets as rows over `/platform/thing/Bottle` (`keg`, `cask`, `wine-bottle`, `can`, `mixer-bottle`, `ice-bag`) or `/platform/thing/Crate`, the floor product at target in the outfit's stock, the outfit trio (`<x>-outfit` Business + `<x>-stock` + `<x>-hand` on `consigns` → the distribution counter), the serving recipe (`pint`, `glass-of-{red,white,sparkling}`, `soft-drink`). | `<x>` | the floor + outfit (producer) |
