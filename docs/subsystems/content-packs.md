...
| **trade-hearth-cooking** (+ the pantry, libations) | platform, generic-objects, trade-distilling | default | `/trade/hearth-cooking` → group `hearth-cooking` (PM-owned); the four wave-4b recipes + `simple-syrup`; the pantry: `sugar`/`salt`/`coffee`/`simple-syrup` materials, the `sack` preset, sacks + a `syrup` floor row, the `syrup-bottle`, the pantry outfit (`pantry-outfit` + `pantry-floor` + a consigning hand) | `hearth-cooking` | the pantry floor + outfit (producer) |
| **trade-distilling** (CAPABILITY — libations) | platform, base-library, generic-objects | default | `/trade/distilling` → group `distilling` (PM-owned): `src/` ships `SpiritBottle` (a `Bottle` preset in code) and `Still` (the furnace-family station, capability `still`, no recipe yet); ten spirit materials tagged by category; ten generic floor bottles at target + Crowsfoot (`gradeBand: fine`, the independent Brand row at `/stuff/idea/corpo/Brand/crowsfoot-gin`); the **cash-and-carry** (the distributor the bar buys from — a `Stock` counter with no lines = a consignment shelf, a Business with `clerk` + `keeper`, a clerk on `shifts`); two floor outfits with `consigns` hands; the `warehouse` bundle; the `distilling` Discipline | `distilling` | the three rooms + the three Businesses (producer) |
| **trade-brewing** · **trade-winemaking** · **trade-bottling** · **trade-produce** (STUB trades — libations) | platform, base-library, trade-distilling | default | `/trade/<x>` → group `<x>` (PM-owned): everything **downstream of production and nothing of production** — materials tagged by category (`carbonated` on sparkling and the sodas; bottling's `ice` = frozen water with `density`/`meltingPoint`/`latentHeatOfFusion`), the vessel presets as rows over `/platform/thing/Bottle` (`keg`, `cask`, `wine-bottle`, `can`, `mixer-bottle`, `ice-bag`) or `/platform/thing/Crate` (`crate`, `basket` — produce's fruit are `Provision` rows the crate populates), the floor product at target in the outfit's stock, the outfit trio (`<x>-outfit` Business + `<x>-stock` + `<x>-hand` on `consigns` → the distilling counter), the serving recipe (`pint`, `glass-of-{red,white,sparkling}`, `mixer`). Produce also homes the growing cluster (`thing/{crop,seed,plant,pot,bed}/`) drained from generic-objects | `<x>` | the floor + outfit (producer) |
| **trade-hospitality** (CAPABILITY — libations) | platform, base-library, generic-objects, the five stubs, hearth-cooking | default | `/trade/hospitality` → group `hospitality` (PM-owned): `src/` ships `IceBin` (an insulated Thermos of ice) and `Tap` (a Surfaced fixture that is a `tap` tool); the bar tools (muddler, bar-spoon, strainer, juicer), the stations (tap, ice-bin, water-tap, basin, glass-rack, well, house-tablet), the nine pool glasses over `CraftedDrink` (coupe = cocktail-glass renamed) + the juice bottle, the four house-made juices; 21 cocktails + coffee + four presses; the `bar` and `cellar` bundles; `archetypes/hospitality.yaml`; `menu.test.ts` materializes a venue from the archetype and orders all 24 lines | `hospitality` | — |
...
| **corpo-veshko** (libations: the one corpo that MAKES) | platform, trade-distilling | organization `/corpo/veshko` | `/corpo/veshko`; `/world/veshko` (industrial) — the yard: `content/world/veshko.yaml` (the first zone to author `stocks:` — vodka 24, whiskey/rum/gin 12), `idea/business` (`parentOrganization: /corpo/veshko`, a `hand` with `purchases`), `thing/stock`, `agent/hand` (`consigns`), `location/distillery`, the `volk` bottle + Veshko's unbranded whiskey/rum/gin | — | `/corpo/veshko`, the distillery, the business (producer) |
| **corpo-hollis** (libations: PRIVATE-LABELS Veshko's liquid) | platform, trade-distilling, corpo-veshko | organization `/corpo/hollis` | `/corpo/hollis`; `/world/hollis` (industrial) — a bottling floor that distils nothing: `old-hollis` / `hollis-cane` rows whose `interiorMaterial` is distilling's whiskey / dark rum (the private-label fact, legible on `look`), the outfit trio | — | `/corpo/hollis`, the floor, the business (producer) |
...
trade-brewing, trade-winemaking, trade-bottling, trade-produce and made
