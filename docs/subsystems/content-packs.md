...
## ⭐ The `worldScan` convention — a system pack's one indexed read

A capability pack cannot ship an Api or a logic singleton, so it cannot
be named in the kernel's `RegistryWideReaders` list the way
`/platform/idea/api/<feature>` entries are — and requiring a kernel edit
for each new system pack is exactly what the pack system exists to
prevent.

The pair list therefore carries **one convention-shaped entry**:

```
FromTemplateMethod('/system/*/idea/*Catalogue', 'worldScan')
```

Any system pack's catalogue may take its registry-wide read from a
method called `worldScan`, with no kernel edit. It is bounded like every
other admitted reader: only the shape the composition index answers
(`world:[mixin.X]`), so what a pack gains is a cheap indexed read of its
own declared population, never a walk of the world. Today's one consumer
is `WatercourseCatalogue`.
...
| **trade-tailoring** (CAPABILITY — fit, textiles B4) | platform, base-library, generic-objects, trade-textiles, terminus | default | `/trade/tailoring` → group `tailoring` (PM-owned): `src/` ships `CutPieces` (Wearable *before* it is wearable, so the `cutTo` stamp survives cut-today-sew-tomorrow), `CuttingTool` (shears + the cutting table as two rows) and `SewingTool` (a needle-case; extends `MendingTool` so it mends too) — ⚠ the verbs are on the INSTRUMENT, not on the furniture they were wrongly declared on for a whole build — `MeasureBook` (⚠ an OBJECT, not a document — `DOCUMENT_KINDS` is a closed kernel vocabulary a pack may not extend, and a ledger on the counter is the better fit for "it transfers with the shop"), the `cut`/`sew`/`alter` controllers and the `measure figure` stanza controller, plus the `tailors` demonstrator brain. It receives the `leather-jerkin` recipe from `trade-smithing`, ⚠ still unmakeable — its hide has no producer and inventing a faucet is forbidden | `tailoring` | the shop + the Business (producer) |
...
trade-tailoring; the world-scan build gave **water** and **residence**
one each — water's two declaring mixins, and residence's
`ResidenceCatalogue` plus the `maintains` brain in its own
