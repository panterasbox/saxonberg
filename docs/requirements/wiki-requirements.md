...
✅ **UNBLOCKED 2026-08-03.** `reference-lifetime` merged. `lib/mixin.ts`
exports `FieldMetaEntry` carrying `authorable` / `authorPicker` /
`runtimeState` alongside the persistence and reference axes, and
`MixinApi.getAllFieldMeta` collects it with **property-level merge** up
the composition chain — so a subclass adding `spoiler` to a field its
base declares `persistent` gets both.

`spoiler` is therefore one more property beside its siblings, needing no
new mechanism, and `spoilerLevelOf(ctor, field)` is the single seam where
the fail-open default lives.
