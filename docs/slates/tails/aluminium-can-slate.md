...
| **temperature** | a cold can from cold storage | **built** — `Thermal` on `Bottle`; the cellar is a room authored at 285 K and anything carried in drifts to it (`restamp` → `resolveTemperatureFor`) | the powered walk-in, when the power meter is worth having | cold storage is a property of a SPACE, not an appliance — and the cellar/walk-in split is a real venue tradeoff (free and never fails vs. colder but costs power and dies with it) |
...
[standing-mint-slate.md](../builds/standing-mint-slate.md). What stays here is
the can.*
...
- **Authorship** — players draft the law as content
  ([legal-code-slate.md](../builds/legal-code-slate.md)); writing a bottle bill
