...
- [weather-slate](./tails/weather-slate.md) — **shipped (Wave 1)** → [weather.md](../subsystems/weather.md). Atmospheric *dynamics* (the procedural driver over biome's static state; thermal's dynamic source): the stateless weather grammar, the SkyExposed biome-deviation seam, the presence-gated thermal coupling, and `analyze weather`. Per-locality off the shipped addressing tree (global until Localities are authored). Slate now a tail holding the Wave 2 "teeth" (**being consumed by storms-and-wetness below**).
- [storms-and-wetness-slate](./builds/storms-and-wetness-slate.md) — **weather Wave 2 promoted to a build** (the Storm frontier for [capability-magic](./deferred-rpg/capability-magic-slate.md), sibling of the shipped electricity Lightning-frontier). Introduces a new cross-cutting **wetness substrate** (a per-object drying gauge — read by electricity/thermal/fire) and gives storms teeth. **Spine = the authored↔procgen coexistence resolve** (one resolved atmospheric state; precedence **author-pin > procgen > climate-lean > biome**; two pin tiers [Locality + scope] × two flavors [frozen + pinned-but-alive]; the line *weather = sky dynamics / authored atmosphere = anywhere*). **v1:** the resolve + wetness gauge + precipitation→`Floor` puddle accumulation (→ conductive puddles, the electricity tie) + **storm lightning strikes** (an `EnergizedMixin` source firing `conduct` — the mundane Create·Lightning proof) + cloud→light dimming. Deferred: fog→visibility, snow depth, vector wind, wet-firewood/fire, the far economy.

**Phases:** vitals substrate (built → tails) → encumbrance (built → tails) → metabolism (built → tails) → respiration (built → tails) → **thermal (built → tail)** → **weather Wave 1 (built → tail)** → **storms-and-wetness (Wave 2 — next; wetness substrate + Storm frontier)**.
