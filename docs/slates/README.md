...
| [kick-relay](./tails/kick-relay-slate.md) | streaming.md | the **third transport**: Kick tune-in parity (`watch` embed + read-only chat relay via the official **webhook**-delivered event API — the first inbound receiver in the streaming stack) + Kick as a **co-equal auth provider** — login + link, one unified provider interface (`KickProfile`, the TwitchProfile precedent over generic `passport-oauth2` OAuth 2.1/PKCE); posting = a cheap phase 2 (`chat:write` reauth-style upgrade through the existing throttle) |
