...
- [api-boot-retirement-slate](./builds/api-boot-retirement-slate.md) — **NEW 2026-09-01. Not built** (the first conversion — `FermentApi.boot` → a self-warming `FermentProfileCatalogue` — shipped with the fermentation MR as the exemplar). `boot()` is an operator act on the consumer Api surface; the root cause is the template-less `*Logic` tier cannot self-warm. Inventory: Material, Condition, WorldClock, Renown, Record — plus the ⚠ ordering keystone that `AppSettings.warm()` runs AFTER `BootstrapManager.run()`, so manifest postRegisters cannot read dials today.
- [oo-calling-conventions-slate](./builds/oo-calling-conventions-slate.md) — **NEW 2026-09-01. Not built.** Verbs between typed objects belong ON the objects (`from.transferTo(to)`); the Api tier keeps only subjectless services, framework lifecycle around a least-trusted host, the import boundary, and subjectless dispatch. Census: **~95–105 mutating object-verbs across ~20 Apis** (worst: BeliefStore 4/4, Chronicle 4/5, Thermal's `depositHeat` a verified duplicate of the mixin's own method, Party 13, Combat ~15), ~150–200 call sites; ten methods already violate the existing thin-wrapper rule through a lint blind spot (wave 0 hardens it). Five line-calls stay the user's (the ledger-write family foremost).
