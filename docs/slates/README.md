...
- [fire-combustion-slate](./builds/fire-combustion-slate.md) — **next; the Fire channel** (combustion as a driver), the marquee frontier-physics build in the "real channels magic actuates" arc (sibling of the shipped electricity Lightning-frontier + storms Storm-frontier). Ships the **mundane combustion substrate** (the magic Fire school actuates it later, the Create·Lightning precedent): the **fire triangle** (fuel + oxygen + heat) as the whole counterplay, `Material.autoignitionTemperature`/`heatOfCombustion` (real K / MJ·kg⁻¹), a `Combustible` capability + a reconcile-on-read `Burning` state driven by a gated `CombustionApi` (`ignite`/burn/`spread`), ignition **routed through the reserved materials-response `thermal` channel** (no parallel damage path), smoke as an unbreathable bulk emission, and the three honest extinguishers (**water/wet** removes heat, **smother/seal** removes O₂, **fuel-starve** burns out). Composes thermal + materials + bulk + air + **wetness** (wet resists ignition); closes the deferred *wet-firewood*, thermal *smoke/cooking/fire-spread*, and electricity *Joule→fire* tails. Promotes the combustion design already in the [thermal tail](./tails/thermal-slate.md); the `Campfire` furnace object already shipped.
