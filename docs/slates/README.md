...
| [content-packs](./builds/content-packs-slate.md) · [content-pack-units](./builds/content-pack-units.md) | content-packs.md / grouping.md | **installer substrate shipped (waves 0+1, 2026-08-25)**: the `content` collection + boot migration, the `pack_installs` record, the three-way reconcile (keep / update / converge / conflict, pins, adoption bridge, per-pack failure isolation, flat-key check), the `pack` verb suite gated on the office-owned `pack-installers` committee, four packs incl. newbie-wilds (the first locality) → content-packs.md; deferred: new contribution kinds (wave 2), pack zero / boot manifest / seeder retirement (wave 3), the `/domain/`→`/world/` + `/trade/` path renames + hearthworks re-cut (wave 4), staging, media, the repo split
