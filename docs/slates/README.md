...
- [dossier-slate](./builds/dossier-slate.md) — ⭐⭐⭐ **priors for a character who never lived them**, and cast-archetype's Change 2 given a home. Derive-on-read is honest for a player who accumulates real events and leaves an authored NPC reading as its floor everywhere — **Dave the Barkeep is a novice bartender, and `advancement.md` says outright there is no way to state otherwise.** The pattern already exists twice (chronicle's `kind: 'deed' | 'claim'`, trait's `dispositions: ClaimSeed[]`) with **two homes for one idea**, and the governing principle is theirs: ⭐ **seed EVIDENCE, not a stat** — the claim marker is what lets the derivation stay pure instead of being overridden. Scorecard: 2 solved · 2 simply missing (competence, the body) · ⚠⚠ 3 *materialized* (renown/participation/influence — seeding the log is a no-op until a fold, a warning copy-pasted into three docs that already cost a live drive). ⭐⭐⭐ **`standing` is not renown — it is `max(0, renownOf) × participationOf`**, so an NPC gets **renown** (seedable, a fact about the world) and never **participation** (nobody turns up on its behalf), and its Compact standing is therefore **zero by arithmetic, with no rule enforcing it.** ⭐⭐ Its best property comes straight from the provocation — *what things **would** aggregate to if real records were modelled* makes a dossier **falsifiable**, so `lint:dossiers` can check the author's `asserting:` against what actually derives. ⭐⭐⭐ Also carries **the identity rung** — `Extra` vs `Cast` as two classes, where the difference is exactly **entitlement to an individual ledger** and a name/prologue/transcript/dossier are all consequences of it. Every ledger keys on `getIdentityPath()`, so instances of one row share one ledger — and **every NPC row in the tree is instanced exactly once today**, which means individual identity holds *by accident* and the extra class is a safety rail rather than a downgrade. ⚠ `Extra` because `props:`/`cast:` already mean something one level down (a role-filler is `Behaved` and goes under `cast:`). ✅ **Settled: an extra's deeds — and harms done TO it — attribute to the INSTITUTION; where there is none, nothing is written** (so an institution's harm rows become a casualty list, and attacking its personnel is a legible act against it) — and the mechanism is a **second derived attribution** beside the shipped `directedBy` (⚠ an earlier answer said "an identity projection" and was wrong — institution applies to `Cast` too, with two attributions rather than one). ⚠ Not free, and the slate says so: **accountability keys on `getTemplatePath()`, not `getIdentityPath()`**, so seven producer call sites move with it — which also exposes a possible pre-existing sandbox inconsistency worth checking on its own. ⚠ Promotion dropped as a runtime idea (identity is a stamp that re-keys the registry index) — promoting an extra means authoring a Cast row for it.
