...
**deferred** with the skill system. But a fourth vector *isn't* deferred:
...
- **The quality mechanics.** How the measurable property bundle is modeled,
  how the skill lookup scatters/tightens it, and how an observer renders the
  *verdict* — without it ever collapsing into a displayed score. The hard,
  central design problem.
- **Crafting actions / verbs.** How you actually combine inputs and apply
  tools at a venue — the command surface (the served path and the DIY path).
...
- **Recipe knowledge & spread.** Taught / earned / discovered — advancement-
  adjacent.
- **The quality verdict, rendered.** Anchored to measurable substrate, never
  a displayed number — the central honesty problem, deferred until consumers
  (cooking, combat) mint the properties that make it matter.
