...
#include "path.h"
#define LOGFILE EVIL + "dance.log"
#define DAVELOG EVIL + "dave.log"
...
"is not easily satiated, nor does his favor come easily."
        ...
        set( "exits", ([ "south" : "/zone/future/emetic/eternal/tunnels/tunnel02" ]) );
	set( "descs", ([
	"throne":
...
void stop_dance(){
	...
	while(boj){
	   ...
	   boj=read_file(LOGFILE, i);
        ...
        move_object(clone_object(EVIL + "get_name_ob"),whodancing); 
	whodancing=THISO;
}
...
void call_dave(string who){
	...
	vic=find_player(who);
	if(vic) {
	   copy_file(LOGFILE, LOGFILE+time());
	   rm(LOGFILE);
	   if(vic->query_wizard()){
	      tell_object(vic, "Dave tells you: You have been very bad.\n");
	   } else {
              dave=clone_object( EVIL + "dave.c");
