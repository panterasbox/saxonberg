...
long()
{
    int hgt;
    string long;
    hgt=height(THISI)*500-random(height(THISI)*50);
    switch(hgt)
    {
    case 1 .. 50 :
	long =
...
{
    call_out("hit_the_ground",time,pancake,2);
}

status
chute_deployed(object buh, int newtime)
{
    move_object(buh,"/zone/null/eternal/school/pfalling");
    "/zone/null/eternal/school/pfalling"->begin_call( height(buh), buh);
    return 1;
}
