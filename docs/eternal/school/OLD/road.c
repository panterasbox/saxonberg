...
#define LOT "/zone/null/eternal/school/parking_lot"
...
void extra_create()
{
    ...
    set("exits",
      ([
	"lot":"/zone/null/eternal/school/parking_lot",
      ]) );
    set("descs",([
	({"road","highway"}):
...
status runOver(object roadkill)
{
    ...
    wot=({"truck","VW Bug","VW Van","Eternal City Bus","Lamborghini Diablo",
      "grey sedan","EotL School bus","steamroller","M1A1 Abrams main battle tank",
      "herd of elephant","moped","popemobile","lime green Ford Pinto",
    });
    ...
    {
    case 0 :
	write(strformat("You're flattened by a passing "+car+"!"));
	tell_room(THISO,strformat(nom+" is flattened by a passing "+car+"!"),({roadkill}));
	if(roadkill->query_resting())roadkill->do_stand();
	roadkill->DeathSequence(THISO,"of massive internal hemorrhaging");
	tell_room(LOT,strformat(
	    "You hear a screech of tires and a thud from the road that sounded "
	    "oddly like "+nom+"."));
	break;
    case 1 :
	write(strformat("You're grazed by a passing "+car+"!"));
	tell_room(THISO,strformat(nom+" is grazed by a passing "+car+"!"),({roadkill}));
	...
	tell_room(THISO,strformat(nom+" shrieks in fear as a "+car+" nearly flattens "+objective(roadkill)+"!"),({roadkill}));
	tell_room(LOT,strformat("You hear a shriek from the road that sounds oddly like "+nom+"."));
	call_out("runOver",random(4)+1,roadkill);
	break;
    case 3 :
	write(strformat("A passing "+car+" smashes into "
	    "you and throws you off the road!"));
	tell_room(THISO,strformat("A passing "+car+" smashes into "+nom+" and throws "+objective(roadkill)+" off the road!"),({roadkill}));
	roadkill->take_damage((roadkill->query_hp()-1),0,"blunt",THISO);
	move_object(roadkill,LOT);
	roadkill->glance();
	tell_room(LOT,strformat(nom+"'s broken, deformed body hurtles onto the lot and rolls to a stop!"),({roadkill}));
	break;
    case 4 :
	write(strformat("The breeze from a passing "+car+" in "
	    "the other lane knocks you off your feet!"));
	tell_room(THISO,strformat(nom+" is knocked off "+possessive(roadkill)+" feet "
	    "by the breeze from a passing "+car+"!"),({roadkill}));
