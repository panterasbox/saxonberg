...
#define MIDAIR SCHOOL+"mid_air"
#define FALLING SCHOOL+"falling"
#define PFALLING SCHOOL+"pfalling"
...
status pull_rip(string wot)
{
	...
	tell_room(ENV(THISI),PRNAME+" pulls on the ripcord of "+possessive(THISI)+" parachute and "
	...
	tell_room(ENV(THISI),
	  PRNAME+" yanks on the ripcord of "+possessive(THISI)+" parachute and it deploys!\n"
	  "Unfortunately, "+PRNAME+" wasn't wearing the chute and it flies off into the sky.\n",({THISI}));
	destruct(THISO);
	return 1;
    }
    SCHOOL+"falling"->chute_deployed(THISI);
    tell_object(THISI,"You yank on the ripcord and the chute deploys!  You drift peacefully towards the ground.\n");
    tell_room(ENV(THISI),PRNAME+" yanks the ripcord on "+possessive(THISI)+" parachute and the chute deploys!  "+objective(THISI)+" begins to drift peacfully towards the ground.\n",({THISI}));
    THISI->set("chute",1);
    set("packed",0);
    return 1;
}
...
status do_pack(string wot)
{
    ...
    tell_room(ENV(THISI),
      PRNAME+" lays "+possessive(THISI)+" chute on the ground and folds it carefully into its pack.\n",
      ({THISI}));
    return 1;
}
