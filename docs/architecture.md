...
| `lib/combat/` | `CombatantMixin` | the "I can fight" capability: the `attack` + `fight` command affordances, the species-declared innate-attack hook (`naturalAttackChannel` → `getNaturalAttackChannel`), and a banded `look` augmenter (the combat-state line while a fight is live). Carries **no** transient fight state — poise/tempo/flags all live on the `CombatSession`, never on the `Creature`. Composed by `Character`. See [combat.md](./subsystems/combat.md). |
