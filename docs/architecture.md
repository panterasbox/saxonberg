...
| `lib/slot/` | `ProstheticMixin` | a worn `Wearable` that stands in for a MISSING body part (recovery build): `forParts` + `restores`, a candidate-side `fitsSlot` that refuses a whole body. Function is DERIVED by `Vitals.ownFunction`, never stored — the peg-leg/hook-hand. The concrete `Prosthesis` class + rows are `trade-medicine`'s. See [embodiment.md](./subsystems/embodiment.md). |
