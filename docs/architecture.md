...
- **Named value-object / vocabulary / registry modules** — the
  ...
  `Mixins`, `lib/paths.ts`'s `TemplatePaths`). ⭐ `lib/supply/SupplyState.ts`
  (the water build) is the clearest case for why the *kernel* is the home:
  six words (`dry` · `cut` · `frozen` · `fouled` · `off` · `overdrawn`), a
  precedence order and a reporting interface, with **no logic at all** —
  it lives here because two different packs have to fail in the same
...
| **pack** | `packages/content/<pkg>/src/**` (tests unrestricted) | `@saxonberg/server/<subpath>` only where the server's `exports` map serves it (the pack import profile: `mud/lib/*`, `mud/api/*`, `mud/platform/{thing,idea,agent,location}/*` minus `platform/idea/api/**` + `hooks/**`), `@saxonberg/types`, a pack it depends on; relative imports inside its own `src/`; no Node built-ins — see content-packs.md § The capability rung |
...
| `lib/vitals/` | `PrescriptionMixin` | the physical prescription slip — patient-bound (`isFor`), dose-counted (`spendDose`/`getDosesLeft`), for a controlled active; re-prescribe clones a fresh one (the lost-scrip replacement flow). On `Prescription` ([blood.md](./subsystems/blood.md)) |
| `lib/calendar/` | `CalendarMixin` | the personal calendar a body carries on its aether implant — dated entries medicine writes the follow-up date into; `calendar` reads it and ⚠ `assess` never leaks a future date. On `Avatar` ([calendar.md](./subsystems/calendar.md)) |
| `lib/calendar/` | `CalendarAppMixin` | the hosted calendar app (`CalendarUpdate`) that surfaces the calendar on the implant ([calendar.md](./subsystems/calendar.md)) |
| `lib/craft/` | `ComminutingMixin` | grinding as CONTINUOUS extraction: `planComminution` splits a charge into product and residue by an extraction fraction against the material's own residue share, stamps grade and mark, and reads its power through `availablePowerW()` — 0 on the kernel, the pack's river on a water mill (`settlePower()` first). The stamp mill is its second consumer, which is why it is kernel (bulk.md) |
| `lib/material/` | `AlloyedMixin` | the per-instance MINOR CONSTITUENTS of a piece of metal — `alloying: CompositionEntry[]` + a `temper` word, in the same `{materialPath, fraction}` vocabulary `Material.composition` speaks, with `getEffectiveComposition()` folding them over the row's and `setFractionOf()` (zero REMOVES). ⭐ Carbon is `fractionOf(carbon)`, and the whole ferrous ladder is that one scalar against two thresholds. ⚠ On metal STOCK only — `Ingot` · `Casting` · `Bloom`: a Material is a singleton so it can say what steel IS and not how yours came out, and carbon on a BLADE would make every mint guard for it (the blade's metal is its Material row). `MixinApi.isAlloyed`. See [crafting.md](./subsystems/crafting.md) |
...
| `lib/ground/` | `ImprovableMixin` | *this ground can be brought into heart* — the closed job vocabulary (`clearing` · `draining` · `liming`), the four percept bands (`rough` · `broken` · `worked` · `in-heart`, never a number), the reversion rates and `ImprovementCost`. ⭐ Promoted out of `trade-farming` by extraction W2 on the documented test — its composers (a field, a turbary) have **no common pack ancestor**. Three host hooks for what the kernel cannot know: `improvementBill()` (⚠ returns `null`, so a host that answers nothing is visibly broken rather than silently free) · `improvementPace(job)` · `improvementSpoils(job)`. See [ground.md](./subsystems/ground.md). |
| `lib/ground/` | `Workable` / `Diggable` / `Splittable` | ⚠ **shapes, not mixins** — the worked-act protocol `dig` and `split` speak: `planWork` (pure, answers a plan or a refusal discriminated on `kind`) then `completeWork` (once, at engagement completion, with the plan's own opaque token). Two phases because a swing is an engagement and a barge-in must leave the ground as it was. ⭐ The ground prices its own pace, refuses in its own words, mints its own product and names the Discipline to credit (`WorkResult.credit` — the one field that lets a platform verb earn a *trade's* competence), which is why `trade-quarrying` ships no verbs and no controllers. See [ground.md](./subsystems/ground.md) § The worked-act protocol. |
| `/system/ground` `lib/` | `StrataMixin` | the five position reads — `getCell` · `metresOf` · `getDeposit` · `getGroundSeed` · `sampleHere` — lifted out of `trade-mining`'s `WorkingMixin`, which now composes over it. Knowing where you are in the column is the ground's business; cutting it is the trade's. See [mining.md](./subsystems/mining.md). |
...
| `lib/perception/` | `AmbientLitMixin` | **inherent ambient light a scope exposes**, and since the envelope build it is DERIVED: `AmbientSource = 'sky' \| 'glow' \| 'none'` with `null` meaning *derived* — a scope open to the sky follows the sun because its biome says so, and one that is not has no ambient at all. `ambientIntensity` survives only as a **noon calibration**, `ambientOpening` names the detail a `'sky'` interior gets its daylight through, and `skyNoonFlux()` × the sky factor × the weather dim is the walk's leg (a). Composed at the `Location` base. See [light.md](./subsystems/light.md). |
| `lib/perception/` | `PublicLightingMixin` | **street lighting as a property of the street** — a row declares the service (`flux`, a `detail` id, a `seniority`) and whether it burns is derived from the hour and from whether the covering extent paid tonight. **No lamp object exists anywhere.** ⚠ Composed on `platform/location/Street` and nowhere else: it went on `CartesianLocation` for one build, which gave a cellar and a mine heading a lighting field to answer for. See [light.md](./subsystems/light.md), [civics.md](./subsystems/civics.md). |
| `lib/thermal/` | `SpaceHeatingMixin` | **a fire that warms the room it stands in**, `spaceHeatOutputW()` summed by the envelope's integration. Composed by `Hearth` and `Campfire` and NOT by `Forge` — *a lit forge must not warm the room it stands in* is kept by composition, so nothing anywhere asks "is this a forge". See [thermal.md](./subsystems/thermal.md). |
| `lib/stuff/` | `CostumedMixin` | ⭐ **the third designation beside `props:` and `cast:`** — `costume:`, what a cast member wears, on a body rather than a place. Same rail: an `instruction:` field, a Phase-2 applier, a once-flag, and the class gated on `Mixins.Wearable` **before** anything is cloned. Composed by `NPC`. ⚠ Not `wardrobe` — that is `lib/slot/Wardrobe.ts`, a player's saved named outfits. See [persistence.md](./subsystems/persistence.md) § Props and cast. |
...
| `lib/document/` | `RegistrarMixin` | **a body that keeps a book about somebody else** — declares `registerPrefix` / `registerOwner` / `registerKind`, and `DocumentApi.saveToRegister` derives all three. The generic shape behind *you file; you do not hold the pen*; replaced a pinned `saveHerd` that hardcoded a pack's namespace in the kernel. ⭐ Safe by an invariant rather than an allowlist: **the owner must be a prefix of the register's own template path**, so a register keeps its own book and nobody else's. See [document-store.md](./subsystems/document-store.md). |
...
| `lib/employment/` | `OrganizationMixin` | the **org chart**, lifted out of `BusinessMixin`: appointing authority (a `PrincipalRef` — `entity`/`office`/`seat`/`committee`) + positions + roster + `parentOrganization` nesting. Answers *who holds position P in organization O?* identically for a ministry, a shop and a press office — a thing that does not trade no longer has to pretend to. Composed by `OrganizationEntity` and (via the base constraint below) `BusinessEntity`. Also the LABOR MARKET reads: `openingsFor`/`openings` (derived, `headcount − holders` — nothing stores an opening, so a hire closes it and a `quit` reopens it by arithmetic) and `considerApplicant` → `ApplicationVerdict`, whose every refusal carries `wanted` AND `held`. `MixinApi.isOrganization`. See [employment.md](./subsystems/employment.md). |
...
| `lib/employment/` | `EmployedMixin` | an actor's employment relationships (sparse null-default `employments` store; `isFulfilling(discipline?)` = *am I the on-duty hand here* — on shift, in a seat whose house lists the discipline under `fulfills`, standing somewhere that house operates; `clockOn`/`clockOff` are the actor's own shift acts). ⭐ Nothing is composed to make somebody a maker — an Avatar and an NPC answer identically, which is what makes a PLAYER on shift the resolved maker. Mutators are gated by a **participant contract**, not `ApiOnly`: `FromMixin(Mixins.Organization, …)` — the employing organization is the legitimate caller, and the gate says *who*, not merely *not you*. Composed by `Character` (actor-agnostic; NPCs are the v1 consumer). See [employment.md](./subsystems/employment.md). |
| `lib/employment/` | `OffstageMixin` | the off-shift parking role for a Location — a marker plus the one invariant (never `Exitable`); `Mixins.Offstage`, `MixinApi.isOffstage`. The clonable every venue's `offstage` row names is `platform/location/Offstage`; the `shifts` brain relocates off-duty cast into it (content packs wave 4b). See [employment.md](./subsystems/employment.md). |
...
| Registry-class handoffs (`register<X>RegistryClass`), the security↔shadow slot, the shadow↔command bridge, the glob merge-on-arrival ripple | **`BootstrapManager.installFrameworkWiring()`** — one idempotent boot call, invoked by `BootstrapManager.run()` in production and, in tests, by an explicit `import "…/test-bootstrap"` in each file that needs a wired world (`src/test-bootstrap.ts`; it was a global vitest setup file until the setup tax made it a third of the suite wall-clock — see docs/testing.md). |
