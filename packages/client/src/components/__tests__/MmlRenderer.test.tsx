...
describe("MmlRenderer", () => {
  ...
  describe("per-viewer 'you' substitution", () => {
    function renderWithViewer(text: string, viewerStuffId: string) {
      return render(
        <MmlRenderer
          text={text}
          onCommandClick={vi.fn()}
          onCommandPreview={vi.fn()}
          viewerStuffId={viewerStuffId}
        />
      );
    }

    it("renders 'you' when a <name> stuff-id matches the viewer", () => {
      const { container } = renderWithViewer(
        '<name stuff-id="iffy">Iffy</name> waves at <name stuff-id="bobalu">Bobalu</name>.',
        "iffy"
      );
      expect(container.textContent).toBe("you waves at Bobalu.");
    });

    it("renders the original label when stuff-id does NOT match the viewer", () => {
      const { container } = renderWithViewer(
        '<name stuff-id="iffy">Iffy</name> waves at <name stuff-id="bobalu">Bobalu</name>.',
        "someoneelse"
      );
      expect(container.textContent).toBe("Iffy waves at Bobalu.");
    });

    it("substitutes 'you' inside <mention> tags as well", () => {
      const { container } = renderWithViewer(
        'Bobalu says, "hi <mention stuff-id="iffy">@iffy</mention>".',
        "iffy"
      );
      expect(container.textContent).toBe('Bobalu says, "hi you".');
    });

    it("does NOT substitute 'you' inside <item> tags (items aren't people)", () => {
      const { container } = renderWithViewer(
        'You pick up <item stuff-id="apple">an apple</item>.',
        "apple"
      );
      expect(container.textContent).toBe("You pick up an apple.");
    });

    it("does NOT substitute 'you' inside <location> tags", () => {
      const { container } = renderWithViewer(
        'You stand at <location stuff-id="lobby">Duncan Hall lobby</location>.',
        "lobby"
      );
      expect(container.textContent).toBe(
        "You stand at Duncan Hall lobby."
      );
    });

    it("does nothing when viewerStuffId is undefined", () => {
      const { container } = render(
        <MmlRenderer
          text={'<name stuff-id="iffy">Iffy</name> waves.'}
          onCommandClick={vi.fn()}
          onCommandPreview={vi.fn()}
        />
      );
      expect(container.textContent).toBe("Iffy waves.");
    });
  });
});
