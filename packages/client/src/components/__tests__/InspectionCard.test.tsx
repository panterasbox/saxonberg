...
/**
 * ⚠ The card no longer opens its own subscriptions.
 *
 * They live in `useInspectionSubscriptions`, mounted by `CardFeed` —
 * because the card now renders as a card that is SUPPRESSED when it
 ...
 * harness is what `CardFeed` does, so these tests keep exercising the
 * real arrangement rather than one that no longer ships.
 */
function Harness(props: React.ComponentProps<typeof InspectionCard>) {
  useInspectionSubscriptions();
  return <InspectionCard {...props} />;
}
...
/**
 * Pull the subscriptionId the card registered with the wire client.
 * The card mints a UUID; the test needs the exact id to address delta
 * envelopes at the same subscription. Pass the CARD NAME to disambiguate
 * the two.
 *
 * ⚠ This used to key on `spec.query`, because the card used to send MQL.
 * It does not any more — the server owns what a named card subscribes
 * to — so the query is not something the client knows or should.
 */
function currentSubscriptionId(card: string = "inspect"): string {
  const c = websocketClient as unknown as {
    mqlSubscriptions: Map<
      string,
      { subscriptionId: string; spec: { card?: string } }
    >;
  };
  for (const entry of c.mqlSubscriptions.values()) {
    if (entry.spec.card === card) return entry.subscriptionId;
  }
  throw new Error(`no subscription registered for card ${card}`);
}
...
describe("InspectionCard", () => {
  ...
  it("mounts with cleared body and a placeholder", () => {
    ...
    const placeholder = screen.getByLabelText("paint card body");
  ...
  it("subscribes to the focus + location queries on mount and unsubscribes on unmount", () => {
    ...
    const cards = subscribes.map(
      (m) => (m as { payload: { card?: string } }).payload.card
    );
    // ⭐ The card opens `inspect` and `location` BY NAME. It sends no
    ...
    expect(cards).toContain("inspect");
  ...
  it("Refresh button sends `look` through the command sink", () => {
    ...
    fireEvent.click(screen.getByLabelText("refresh card"));
  ...
  it("placeholder click sends `look` through the command sink", () => {
    ...
    fireEvent.click(screen.getByLabelText("paint card body"));
    expect(onSend).toHaveBeenCalledWith("look");
  });
  ...
  it("hover preview channel fires for Refresh, breadcrumbs, and contents rows", () => {
    ...
    const refresh = screen.getByLabelText("refresh card");
