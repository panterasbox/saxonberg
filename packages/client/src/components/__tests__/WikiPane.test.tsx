/**
 * WikiPane — the article pane.
 *
 * Two things are worth asserting and the rest is styling: that the
 * pane **renders what the server sent and nothing else** (it has no
...
beforeEach(() => {
  useStore.setState({ wikiPage: null, rightPane: "inspect" });
});
...
describe("WikiPane", () => {
  ...
  it("⭐ shows the body it was SENT — it never re-renders one", () => {
    ...
    // server chose to send is what appears; a pane that could produce
  ...
  it("issues `wiki edit <handle>` from the Edit affordance", () => {
    ...
    render(<WikiPane onSendCommand={onSendCommand} />);
  ...
  it("reading a page brings the pane forward", () => {
    // The verb's purpose is to put an article in front of somebody.
    expect(useStore.getState().rightPane).toBe("inspect");
    useStore.getState().setWikiPage(page());
    expect(useStore.getState().rightPane).toBe("wiki");
  });
});
