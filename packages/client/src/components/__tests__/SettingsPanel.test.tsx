...
/**
 * SettingsPanel — the first summoned-card consumer. Every control sends a
...
describe("SettingsPanel", () => {
  it("degrades the notifications section when the backend is absent", () => {
    render(<SettingsPanel onSendCommand={vi.fn()} onClose={vi.fn()} />);
  ...
  it("a quick-setting select sends the real settings command", () => {
    ...
    render(<SettingsPanel onSendCommand={onSendCommand} onClose={vi.fn()} />);
  ...
  it("the close button calls onClose", () => {
    const onClose = vi.fn();
    render(<SettingsPanel onSendCommand={vi.fn()} onClose={onClose} />);
