/**
 * WikiCard — the right-column cockpit card for reading an article
 ...
 * `services/websocket.ts`). This card reads it reactively.
 *
 ...
 * reveal model rests on there being ONE gate, on the server, and a card
 ...
 *
 * Every affordance is a **command the card composes and emits** — Edit,
 ...
 * card can do can also be typed, and the verb re-checks permission on
 * arrival regardless of what `mayEdit` let the card draw.
 *
 ...
 * contract, which the card obeys for its own affordances as well as
...
export function WikiCard({
  onSendCommand,
  onCommandPreview,
}: WikiCardProps): React.ReactElement {
  const page = useStore((s) => s.wikiPage);

  if (!page) {
    return (
      <CardContainer aria-label="Wiki">
        <Header>
