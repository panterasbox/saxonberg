 ...
 * the whole security argument for having a pane at all: the payload
 ...
 * reach a client (criterion 24). A pane fed from the SOURCE would be
 * exactly that second path, and the leak would be invisible from the
 * scroll, which would still look correct.
 *
 * Everything else here is the pane's raw material: the outline (public
...
describe('a read pushes the pane its article', () => {
  ...
  it('⭐ carries the RENDERED body, not the source', async () => {
    ...
    // carried the source, the pane would be holding text the gate
    // never saw.
  ...
  it('⭐ over-capability content is absent from the PAYLOAD too', async () => {
    // The one that matters. The pane is a second surface; if the
  ...
  it('carries the frontmatter the pane displays', async () => {
    await seedArticle();
    frames = [];
    await run({ subcommand: 'read', page: 'oak' });
  ...
  it('answers `mayEdit`, so the pane does not draw a refused affordance', async () => {
    await seedArticle();
    frames = [];
    await run({ subcommand: 'read', page: 'oak' });
...
describe('a preview says it is one', () => {
  it('flags the frame, and its outline comes from the DRAFT', async () => {
    // Otherwise the pane's outline describes the saved page while its
