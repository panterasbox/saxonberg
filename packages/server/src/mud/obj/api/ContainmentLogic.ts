 ...
 * Lives at `/obj/api/containment`; `ContainmentApi`'s statics forward
 * here via `StuffApi.singletonSync`. `dest /obj/api/containment`
 * reloads it.
 *
 * Gating (the guts-variant recipe): every public method carries
 * `AnyOf(FromModule('mud/api/containment#ContainmentApi'), SelfOnly)`.
