...
const REGISTRY_PATH = '/obj/CardRegistry';
 ...
 *
 * Lives at `/obj/api/card` (a stateless `Stuff` singleton, no backing
 * `Template`); the statics on `CardApi` forward here via
 * `StuffApi.singletonSync`. All card state lives on the `CardRegistry`
 * at `/obj/CardRegistry`, resolved via the module-level
 * `resolveRegistry()` — which survives this singleton's
 * destruct/recreate across HMR, so `dest /obj/api/card` reloads the
