 ...
 * `/obj/Topic/<key>` row like any other content and it just works, with
...
function writeTopicPack(id: string, topicKey: string): string {
  ...
  const file = join(root, 'content', 'obj', 'Topic', `${topicKey}.yaml`);
...
function topicRows(): Row[] {
  return rows.filter((r) => String(r.path ?? '').startsWith('/obj/Topic/'));
}
...
describe('pack-declared topics', () => {
  it('accepts a leaf under a core root', async () => {
    ...
    expect(topicRows().map((r) => r.path)).toEqual(['/obj/Topic/sense.pollen']);
  });
