...
describe('CombatApi.blameFor delegation — the pinned migration regression', () => {
  it('a consented duel kill is NOT a crime (read through CombatApi)', async () => {
    await seed({
      kind: 'death',
      victim: '/obj/Avatar/duelist',
      lethality: 'lethal',
      consented: true,
      sentient: true,
      realAt: 10,
    });
    const v = await CombatApi.blameFor('/obj/Avatar/duelist');
  ...
  it('an imposed lethal kill IS a crime (read through CombatApi)', async () => {
    await seed({
      kind: 'death',
      victim: '/obj/Avatar/townsperson',
      lethality: 'lethal',
      consented: false,
      sentient: true,
      realAt: 10,
    });
    const v = await CombatApi.blameFor('/obj/Avatar/townsperson');
  ...
  it('null when the victim has no attributed death', async () => {
    expect(await CombatApi.blameFor('/obj/Avatar/nobody')).toBeNull();
  });
});
...
describe('AccountabilityApi — the trap (harm) producer read', () => {
  it('a harm row on a non-consenting sentient derives a crime', async () => {
    ...
    expect(await AccountabilityApi.crimeFor('/obj/Avatar/passerby')).toBe(true);
    const v = await AccountabilityApi.blameFor('/obj/Avatar/passerby');
