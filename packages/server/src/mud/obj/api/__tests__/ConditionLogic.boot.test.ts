...
describe('ConditionApi.boot — the roster warm', () => {
  it('stands up Condition rows and skips folder rows', async () => {
    vi.spyOn(Template, 'findDescendants').mockResolvedValue([
      { path: '/obj/Condition/metabolism', class: '/obj/FolderZone' },
      { path: '/obj/Condition/metabolism/alcohol', class: '/obj/Condition' },
      { path: '/obj/Condition/thermal/hypothermia', class: '/obj/Condition' },
    ...
    expect(stood).toEqual([
      '/obj/Condition/metabolism/alcohol',
      '/obj/Condition/thermal/hypothermia',
    ]);
  });

  it('tolerates one failed standup and continues (the MaterialApi.boot shape)', async () => {
    vi.spyOn(Template, 'findDescendants').mockResolvedValue([
      { path: '/obj/Condition/metabolism/bad', class: '/obj/Condition' },
      { path: '/obj/Condition/metabolism/good', class: '/obj/Condition' },
    ] as unknown as Template[]);
...
describe('⭐ the real-seed coverage — driven off the seed files, not a mock', () => {
  ...
  it('⚠ the paths the toxin fixtures fabricate are REAL seeded paths', () => {
    ...
    for (const p of [
      '/obj/Condition/metabolism/alcohol',
      '/obj/Condition/metabolism/ptomaine',
    ]) {
