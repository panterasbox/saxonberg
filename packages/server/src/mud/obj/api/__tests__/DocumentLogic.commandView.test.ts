...
const PATH = '/cmd/perception/look';
const LOOK = {
  verbs: ['look'],
...
beforeEach(() => {
  ...
  actor = makeStuffAtPath(() => new Idea(), '/obj/Avatar/ann') as unknown as Stuff;
});
...
describe('command-view saves', () => {
  ...
  it('a non-wizard changing controller is refused', async () => {
    vi.spyOn(AccessApi, 'isWizard').mockResolvedValue(false);
    await expect(
      runAs(() => DocumentApi.save(PATH, 'command-view', { ...LOOK, controller: '/obj/command/system/PingController' })),
  ...
  it('a non-wizard minting a NEW view that names a controller is refused', async () => {
    vi.spyOn(AccessApi, 'isWizard').mockResolvedValue(false);
    await expect(
      runAs(() => DocumentApi.save('/cmd/system/zap', 'command-view', { verbs: ['zap'], controller: '/obj/command/system/PingController', description: 'x' })),
    ).rejects.toThrow(/wizard code trust/);
  });

  it('a wizard changing controller is admitted', async () => {
    vi.spyOn(AccessApi, 'isWizard').mockResolvedValue(true);
    await runAs(() => DocumentApi.save(PATH, 'command-view', { ...LOOK, controller: '/obj/command/system/PingController' }));
    expect((rows[0]!.data as { controller: string }).controller).toBe('/obj/command/system/PingController');
    expect(reload).toHaveBeenCalledWith(PATH);
  });
