...
export default class ReactionRegistry extends Idea {
  ...
  public onScopedEmote(req: ScopedEmoteRequest): ScopedEmoteDecision {
    ...
    const existing = act.reactions.get(reactorId);
    let flippedOn = false;

    if (existing && existing.present && existing.emote === req.verb) {
      // Toggle off: same (reactor, emote). No renown on un-reacting.
      existing.present = false;
    } else {
      act.reactions.set(reactorId, {
        reactorId,
        emote: req.verb,
        emoji: req.emoji,
        tags: req.tags,
