...
>(Base: TBase) {
  return class AtmosphericMixin extends Base
    ...
    // ──────── the weather locality cache (extraction build) ─────────
    // ⭐ Disjoint from the envelope block above: that one integrates a
    // temperature, this one memoizes WHOSE weather it is. They landed at
    // the same insertion point on two branches, which is the whole of
    // why this was a 460-line conflict and none of it was a real one.
