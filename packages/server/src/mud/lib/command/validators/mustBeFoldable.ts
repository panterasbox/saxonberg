/**
 * mustBeFoldable — the bound target (or its door) must compose `isFoldable`.
 *
 * The `fold` / `unfold` gate.
 *
 ...
 * `MixinApi.isFoldable(stuff)` check would pass the first and REFUSE the
...
const validator: FieldValidator = (value, _field, _context) => {
  ...
  const found = MqlApi.effectiveTarget(
    match,
    (s): s is Stuff & object => MixinApi.isFoldable(s),
  );
  if (!found) return `${match.stuff.getPresentation()} doesn't fold`;
  return undefined;
};
