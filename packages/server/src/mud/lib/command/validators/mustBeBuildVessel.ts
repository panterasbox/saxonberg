/**
 * mustBeBuildVessel — every bound Stuff must compose `isBuildVessel`.
 *
 * The by-hand build gate — `heat`, `quench`, `stir`, `strain`, `plate`, `garnish`, crafting `pour`.
 *
...
const validator: FieldValidator = (value, field, _context) => {
  ...
  for (const stuff of stuffs) {
    if (!MixinApi.isBuildVessel(stuff as Stuff)) {
      return `${stuff.getPresentation()} isn't a vessel you can work in`;
    }
  }
  return undefined;
};
