/**
 * mustBeCharged — every bound Stuff must compose `isCharged`.
 *
 * The `recharge` / `zap` item gate.
 *
...
const validator: FieldValidator = (value, field, _context) => {
  ...
  for (const stuff of stuffs) {
    if (!MixinApi.isCharged(stuff as Stuff)) {
      return `${stuff.getPresentation()} holds no charge`;
    }
  }
  return undefined;
};
