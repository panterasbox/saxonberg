/**
 * mustHaveVitals — every bound Stuff must compose `isVitals`.
 *
 * The body gate — `attack`, `fight switch`, `assess`, `treat`, `undress`. ⭐ This is what stops `attack` being offered on a room.
 *
...
const validator: FieldValidator = (value, field, _context) => {
  ...
  for (const stuff of stuffs) {
    if (!MixinApi.isVitals(stuff as Stuff)) {
      return `${stuff.getPresentation()} isn't a living thing`;
    }
  }
  return undefined;
};
