/**
 * mustBeSealable — the bound target (or its door) must compose `isSealable`.
 *
 * The `open` / `close` gate.
 *
 ...
 * `MixinApi.isSealable(stuff)` check would pass the first and REFUSE the
...
const validator: FieldValidator = (value, _field, _context) => {
  ...
  const found = MqlApi.effectiveTarget(
    match,
    (s): s is Stuff & object => MixinApi.isSealable(s),
  );
  if (!found) return `${match.stuff.getPresentation()} doesn't open and close`;
  return undefined;
};
