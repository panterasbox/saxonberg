/**
 * mustBeGlobbable — every bound Stuff must compose `isGlobbable`.
 *
 * The `bank deposit` coin gate.
 *
...
const validator: FieldValidator = (value, field, _context) => {
  ...
  for (const stuff of stuffs) {
    if (!MixinApi.isGlobbable(stuff as Stuff)) {
      return `${stuff.getPresentation()} isn't a countable stack`;
    }
  }
  return undefined;
};
