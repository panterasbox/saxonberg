/**
 * mustBeDurable — every bound Stuff must compose `isDurable`.
 *
 * The `repair` / `hammer` / `analyze response` gate.
 *
...
const validator: FieldValidator = (value, field, _context) => {
  ...
  for (const stuff of stuffs) {
    if (!MixinApi.isDurable(stuff as Stuff)) {
      return `${stuff.getPresentation()} has no condition to work on`;
    }
  }
  return undefined;
};
