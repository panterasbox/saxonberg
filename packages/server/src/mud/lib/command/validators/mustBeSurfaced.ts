/**
 * mustBeSurfaced — every bound Stuff must compose `SurfacedMixin`.
 *
 * Used by `put X on Y` and any future surface-only verb so the
 * controller can rely on `getResting` / `canRest` / `placeOn`.
