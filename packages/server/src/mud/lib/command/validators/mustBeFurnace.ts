/**
 * mustBeFurnace — every bound Stuff must compose `isFurnace`.
 *
 * The `pump` gate — bellows need a furnace to feed.
 *
...
const validator: FieldValidator = (value, field, _context) => {
  ...
  for (const stuff of stuffs) {
    if (!MixinApi.isFurnace(stuff as Stuff)) {
      return `${stuff.getPresentation()} isn't a furnace`;
    }
  }
  return undefined;
};
