/**
 * mustBeLockable — the bound target (or its door) must compose `isLockable`.
 *
 * The `lock` / `unlock` gate.
 *
 ...
 * `MixinApi.isLockable(stuff)` check would pass the first and REFUSE the
...
const validator: FieldValidator = (value, _field, _context) => {
  ...
  const found = MqlApi.effectiveTarget(
    match,
    (s): s is Stuff & object => MixinApi.isLockable(s),
  );
  if (!found) return `${match.stuff.getPresentation()} has no lock`;
  return undefined;
};
