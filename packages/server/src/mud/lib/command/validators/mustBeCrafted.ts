/**
 * mustBeCrafted — every bound Stuff must compose `isCrafted`.
 *
 * The `salvage` gate — you can only break down what was built.
 *
...
const validator: FieldValidator = (value, field, _context) => {
  ...
  for (const stuff of stuffs) {
    if (!MixinApi.isCrafted(stuff as Stuff)) {
      return `${stuff.getPresentation()} wasn't crafted`;
    }
  }
  return undefined;
};
