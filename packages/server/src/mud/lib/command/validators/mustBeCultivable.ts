/**
 * mustBeCultivable — every bound Stuff must compose `isCultivable`.
 *
 * The `plant` / `repot` / `feed` / `water` bed gate. A pot is a bed with one slot.
 *
...
const validator: FieldValidator = (value, field, _context) => {
  ...
  for (const stuff of stuffs) {
    if (!MixinApi.isCultivable(stuff as Stuff)) {
      return `${stuff.getPresentation()} isn't ground you can plant in`;
    }
  }
  return undefined;
};
