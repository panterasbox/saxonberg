...
describe("BankingApi escrow — hold / release / revert / close", () => {
  ...
  async function fund(accountId: string, minor: number): Promise<void> {
    await BankingApi.mint(accountId, Money.of(minor, BankingApi.compactCurrency()));
  }

  it("hold → release round-trips issuer → escrow → contractor with the escrow kinds", async () => {
    await fund(ISSUER, 100);
    const held = await BankingApi.escrowHold(ISSUER, CONTRACT, Money.of(60, BankingApi.compactCurrency()));
    ...
    await BankingApi.escrowRelease(CONTRACT, CONTRACTOR, Money.of(60, BankingApi.compactCurrency()));
  ...
  it("hold → revert returns the stake to the issuer", async () => {
    await fund(ISSUER, 100);
    await BankingApi.escrowHold(ISSUER, CONTRACT, Money.of(60, BankingApi.compactCurrency()));
    await BankingApi.escrowRevert(CONTRACT, ISSUER, Money.of(60, BankingApi.compactCurrency()));
  ...
  it("a hold against insufficient funds refuses before any row is written", async () => {
    ...
    const result = await BankingApi.escrowHold(ISSUER, CONTRACT, Money.of(60, BankingApi.compactCurrency()));
  ...
  it("an over-release throws (the per-contract account makes it loud)", async () => {
    ...
    await BankingApi.escrowHold(ISSUER, CONTRACT, Money.of(60, BankingApi.compactCurrency()));
    await expect(
      BankingApi.escrowRelease(CONTRACT, CONTRACTOR, Money.of(61, BankingApi.compactCurrency())),
