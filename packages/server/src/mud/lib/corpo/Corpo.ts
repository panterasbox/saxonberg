 ...
 * `/lib/corpo/Corpo/<key>`, read by the catalogue directly from
 * `template.data` (never cloned as live Stuff). The catalogue
 * (`/obj/CorpoCatalogue`) scans these templates at boot to build its
 * descriptor cache.
 *
...
export default class Corpo extends Idea {
  /** Per-instance template path prefix: `/lib/corpo/Corpo/<key>`. */
