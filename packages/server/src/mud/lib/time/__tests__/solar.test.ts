/**
 * solar.ts tests — pure geometry checked against textbook values
...
describe('solar declination', () => {
  it('is ~0 at the equinoxes (day 0 and Y/2)', () => {
    expect(Solar.declinationDeg(EARTH_LIKE, 0)).toBeCloseTo(0, 6);
    expect(Solar.declinationDeg(EARTH_LIKE, noonOf(Y / 2))).toBeCloseTo(0, 6);
  });

  it('is +tilt at the summer solstice (Y/4), −tilt at the winter solstice (3Y/4)', () => {
    expect(Solar.declinationDeg(EARTH_LIKE, noonOf(Y / 4))).toBeCloseTo(23.5, 6);
    expect(Solar.declinationDeg(EARTH_LIKE, noonOf((3 * Y) / 4))).toBeCloseTo(
      -23.5,
      6
    );
  });
...
describe('day length', () => {
  it('is ~12 hours at the equinox at 42°N', () => {
    const sunrise = Solar.sunriseSecOfDay(EARTH_LIKE, LAT, 0);
    const sunset = Solar.sunsetSecOfDay(EARTH_LIKE, LAT, 0);
  ...
  it('is longer than 12h at the summer solstice, shorter at the winter solstice', () => {
    ...
    const summerLen =
      (Solar.sunsetSecOfDay(EARTH_LIKE, LAT, summer) as number) -
      (Solar.sunriseSecOfDay(EARTH_LIKE, LAT, summer) as number);
    const winterLen =
      (Solar.sunsetSecOfDay(EARTH_LIKE, LAT, winter) as number) -
      (Solar.sunriseSecOfDay(EARTH_LIKE, LAT, winter) as number);
...
describe('solar altitude', () => {
  it('matches 90 − |lat − δ| at local noon across a full year', () => {
    for (let day = 0; day < Y; day++) {
      const t = noonOf(day);
      const dec = Solar.declinationDeg(EARTH_LIKE, t);
      const expected = 90 - Math.abs(LAT - dec);
      const alt = Solar.solarAltitudeDeg(EARTH_LIKE, LAT, t);
      expect(alt).toBeCloseTo(expected, 1); // within ~0.5°
    }
  });

  it('is negative at local midnight (night)', () => {
    expect(Solar.solarAltitudeDeg(EARTH_LIKE, LAT, 0)).toBeLessThan(0);
    expect(Solar.isDay(EARTH_LIKE, LAT, 0)).toBe(false);
    expect(Solar.isDay(EARTH_LIKE, LAT, D / 2)).toBe(true);
  });
});

describe('solar azimuth', () => {
  it('is ~due south (180°) at noon at 42°N on the equinox', () => {
    expect(Solar.solarAzimuthDeg(EARTH_LIKE, LAT, D / 2)).toBeCloseTo(180, 1);
  });

  it('is in the eastern half in the morning, western half in the afternoon', () => {
    const morning = Solar.solarAzimuthDeg(EARTH_LIKE, LAT, D / 3); // before noon
    const afternoon = Solar.solarAzimuthDeg(EARTH_LIKE, LAT, (2 * D) / 3);
...
describe('season', () => {
  it('quarters the year, spring at the vernal equinox', () => {
    expect(Solar.seasonFor(EARTH_LIKE, noonOf(0))).toBe('spring');
    expect(Solar.seasonFor(EARTH_LIKE, noonOf(100))).toBe('summer');
    expect(Solar.seasonFor(EARTH_LIKE, noonOf(200))).toBe('fall');
    expect(Solar.seasonFor(EARTH_LIKE, noonOf(300))).toBe('winter');
  });
});

describe('moon', () => {
  it('is new at t=0 and full at half the synodic period', () => {
    expect(Solar.moonPhase(EARTH_LIKE, 30, 0)).toBeCloseTo(0, 6);
    expect(Solar.moonPhase(EARTH_LIKE, 30, 15 * D)).toBeCloseTo(0.5, 6);
  });

  it('nextFullMoon returns the first full moon strictly after t', () => {
    expect(Solar.nextFullMoon(EARTH_LIKE, 30, 0)).toBe(15 * D);
    // Just after a full moon, the next is one synodic period later.
    expect(Solar.nextFullMoon(EARTH_LIKE, 30, 15 * D)).toBe(45 * D);
  });
});
...
describe('polar guards', () => {
  it('reports polar day at the summer solstice inside the arctic circle', () => {
    const summer = noonOf(Y / 4);
    expect(Solar.sunriseHourAngleDeg(EARTH_LIKE, 80, summer)).toBe('polar-day');
    expect(Solar.sunriseSecOfDay(EARTH_LIKE, 80, summer)).toBeNull();
  });

  it('reports polar night at the winter solstice inside the arctic circle', () => {
    const winter = noonOf((3 * Y) / 4);
    expect(Solar.sunriseHourAngleDeg(EARTH_LIKE, 80, winter)).toBe(
      'polar-night'
    );
  });
  ...
  it('42°N never goes polar', () => {
    for (let day = 0; day < Y; day++) {
      expect(typeof Solar.sunriseHourAngleDeg(EARTH_LIKE, LAT, noonOf(day))).toBe(
        'number'
      );
    }
  });
