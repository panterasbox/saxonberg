 ...
 * mutations (subdivide / transfer). Lives at `/platform/idea/ParcelRegistry`,
 * sibling to `AccessRegistry` / `AddressRegistry` / `OfficeRegistry`.
 *
 * The thin `ParcelApi` facade (and its `ParcelLogic` singleton at
 * `/platform/idea/api/parcel`) are the only legitimate callers — every public
...
const ParcelApiCallers = SecurityPolicies.AnyOf(
  ...
  SecurityPolicies.FromTemplate("/platform/idea/api/parcel"),
);
...
export default class ParcelRegistry extends ParcelRegistryBase {
   ...
   * `/stuff/idea/lounge`), not ground, and none of them should read as cultivable
