/**
 * EmbodyController — the `embody` verb's step model. Drives execute()
...
import EmbodyController, {
  FIELDS,
  FIELD_ORDER,
} from '../EmbodyController';
...
describe('EmbodyController step model', () => {
  let login: Login;
  ...
  beforeEach(() => {
    ...
    login.setCharacterDraft({ realName: 'Bobby' });
  ...
  function run(rest: string): Promise<void> {
    return ctrl.execute({ rest } as EmbodyModelLike, ctx);
  }
  type EmbodyModelLike = CommandModel & { rest?: string };
  ...
  it('species options carry a structured dossier derived from the model', async () => {
    await run(''); // bare embody → full draft state
  ...
  it('bare embody emits species options with everything still missing', async () => {
    await run('');
  ...
  it('setting a field records it and surfaces the dependent options', async () => {
    ...
    const draft = login.getCharacterDraft()!;
  ...
  it('offers he/she/they for a character and refuses it/its', async () => {
    ...
    expect(login.getCharacterDraft()!.pronouns).toBeUndefined();
  ...
  it('re-picking the same species does NOT wipe downstream fields', async () => {
    ...
    expect(login.getCharacterDraft()!.sex).toBe('female');
  ...
  it('rejects an unknown species with a field-scoped error and no pick', async () => {
    ...
    expect(login.getCharacterDraft()!.speciesKey).toBeUndefined();
  ...
  it('a full draft reports nothing missing, and confirm commits', async () => {
    const commit = vi
      .spyOn(EmbodyController.prototype, 'commit')
      .mockResolvedValue(undefined);

    await run('species human');
    await run('sex female');
    const sug = login.getCharacterDraft()!.suggestion!;
  ...
  it('name reroll produces a fresh themed suggestion', async () => {
    await run('species human');
    const before = login.getCharacterDraft()!.suggestion?.name;
    await run('name reroll');
    const after = login.getCharacterDraft()!.suggestion?.name;
  ...
  it('rejects a typed name that fails validation', async () => {
    ...
    expect(login.getCharacterDraft()!.name).toBeUndefined();
