/**
 * EmbodyController — the char-gen verb (`embody`), the real command
 * handler for character creation.
 *
 * Char-gen is a DRAFT STATE MACHINE, not a step flow. Each
 * `embody <field> <value>` (species / sex / name / pronouns / aspiration)
 * validates + sets that field on the Login's `CharacterDraft` and
 ...
 * vs multi-step is a pure client change). `embody confirm` gates on
 ...
 * option here also refuses `embody pronouns it` from the command line,
...
interface EmbodyModel extends CommandModel {
 ...
 *
 * `embody-projection.test.ts` asserts exactly that, by adding a field
...
interface FieldHandler {
  ...
  applicable(draft: CharacterDraft, cfg: CharGenConfig): boolean;
  /** Whether the draft has a value for this field. */
  isSet(draft: CharacterDraft): boolean;
  options(draft: CharacterDraft, cfg: CharGenConfig): CharGenOption[];
  /** The current value as a display string, when set. */
  display(draft: CharacterDraft): string | undefined;
  /** Optional one-line note rendered under the field. */
  hint?(draft: CharacterDraft, cfg: CharGenConfig): string | undefined;
  validate(
    value: string,
  ...
  apply(
    ...
    draft: CharacterDraft,
    cfg: CharGenConfig,
    ctrl: EmbodyController,
  ): Promise<void> | void;
}

/**
 * @internal Exported as a **white-box test seam only**, so
 * `EmbodyController.test.ts` can add a field and prove it reaches the
...
export const FIELDS: Record<CharGenField, FieldHandler> = {
  species: {
    ...
    options: (_d, cfg) =>
      cfg.species.map((s) => {
        ...
        const card = EmbodyController.getSpeciesCard(s.path);
        return {
          value: s.key,
          label: s.label,
  ...
  name: {
    ...
    // apart — it composes `embody name <given> <surname>` from its own
 ...
 * `embody-projection.test.ts` asserts.
 *
...
export default class EmbodyController extends CommandController<EmbodyModel> {
  ...
  async execute(model: EmbodyModel, context: CommandContext): Promise<void> {
    this.ctx = context;
    const login = context.commandGiver as unknown as Login;
    let draft = login.getCharacterDraft();
    if (!draft) {
      draft = {};
      login.setCharacterDraft(draft);
    }
    ...
    await EmbodyController.ensureSpeciesCards(cfg);
    ...
    if (!field) {
      // Bare `embody` → emit the current draft state.
    ...
    if (err) {
      ...
      context.note({
        kind: "controller-rejected",
        reason: "embody-validation-failed",
        detail: err,
      });
      return;
    }
    ...
    // committing is the client's call (a later `embody confirm`).
  ...
  private emitState(
    ...
    // echo (`> embody species dwarf`) already gives terminal feedback.
  ...
  public async commit(
    ...
    //     founding deed is event-singular by trigger (embody fires once
    ...
    await avatar.recordDeed({
      template: "Embodied as {{ name }}, {{ aspirationLabel }}.",
      vars: {
        name: draft.name,
  ...
  private static async ensureSpeciesCards(cfg: CharGenConfig): Promise<void> {
    if (EmbodyController.#speciesCards) return;
    const cards = new Map<
      string,
      { dossier?: SpeciesDossier; illustration?: string }
    >();
    ...
    EmbodyController.#speciesCards = cards;
  }
  ...
  static getSpeciesCard(
    ...
    return EmbodyController.#speciesCards?.get(path);
  }

  /** @internal a test seam: drops the cached embody config so the next read re-loads it */
  static resetConfigCache(): void {
    EmbodyController.#config = null;
    EmbodyController.#speciesCards = null;
  }
}
