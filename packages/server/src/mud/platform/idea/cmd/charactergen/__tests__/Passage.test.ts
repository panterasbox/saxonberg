...
describe('passage — the floor beneath coming back', () => {
  ...
  it('is afforded ONLY by being incorporeal', () => {
    ...
    expect(yaml).toContain('platform/cmd/charactergen/passage.yaml');
  });

  it('carries NO requiresEmbodied — the one verb for the disembodied', () => {
    const view = readFileSync(
      join(__dirname, '../../../../../../../../content/platform/content/platform/cmd/charactergen/passage.yaml'),
      'utf8',
    );
    expect(view).not.toContain('requiresEmbodied');
    expect(view).toContain('controller: /platform/idea/cmd/charactergen/PassageController');
  });
