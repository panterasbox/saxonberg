...
describe('Bulk verbs — command YAML views load + validate', () => {
  it.each([
    ['platform/cmd/bulk/fill.yaml', 'fill', '/platform/idea/cmd/bulk/FillController'],
    ['platform/cmd/bulk/pour.yaml', 'pour', '/platform/idea/cmd/bulk/PourController'],
    ['platform/cmd/bulk/spill.yaml', 'spill', '/platform/idea/cmd/bulk/SpillController'],
    ['platform/cmd/bulk/drink.yaml', 'drink', '/platform/idea/cmd/bulk/DrinkController'],
    ['platform/cmd/bulk/sip.yaml', 'sip', '/platform/idea/cmd/bulk/SipController'],
...
describe('Bulk verbs — fill / sip / drink', () => {
  ...
  beforeEach(() => {
    ...
    coffee = material('/stuff/idea/material/bulk/coffee', 'coffee');
...
describe('Bulk verbs — pour clamp / mismatch / drain; spill', () => {
  ...
  beforeEach(() => {
    ...
    (actor as unknown as { setName(n: string): void }).setName('bob');
    ContainmentApi.move(actor as never, loc as never);
    water = material('/stuff/idea/material/bulk/water', 'water');
    coffee = material('/stuff/idea/material/bulk/coffee', 'coffee');
...
describe('Bulk verbs — drink prose, and the two article rules', () => {
  ...
  function draught(): Material {
    return makeStuffAtPath(() => {
      ...
      return m;
    }, '/stuff/idea/material/potion/drink-test') as unknown as Material;
  }
  ...
  it('a plain material keeps its shipped "the"', async () => {
    const coffee = material('/stuff/idea/material/bulk/coffee2', 'coffee');
