 ...
 * `TemplateApi.saveTemplate(childPath, { class: '/platform/idea/FolderZone', data: … })` (the
...
export default class SubdivideController extends CommandController<SubdivideModel> {
  async execute(
    ...
    try {
      await TemplateApi.saveTemplate(childPath, { class: "/platform/idea/FolderZone", data: {
        name,
      } });
    } catch (err) {
