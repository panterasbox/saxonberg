/**
 * TraceAddressController — handler for `trace address [<location>]`.
 *
 ...
 * substrate's developer debug surface (parallel to `trace
 * atmosphere`).

 ⭐ It was `analyze address`, and moving it is the point: it is not a
 * READING. A reading is a fact about the world that competence resolves
 * and an instrument sharpens; this is the engine explaining its own
 * resolution, which is a diagnostic and belongs beside `affordances` and
 * `errors` on a free `system` verb.
 */
...
export default class TraceAddressController extends CommandController<AnalyzeAddressModel> {
