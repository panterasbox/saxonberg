/**
 * TraceAddressController — `trace address [<location>]` renders the
...
import TraceAddressController from '../TraceAddressController';
...
function installRoster(): void {
  ...
  mint('/stuff/idea/Locality/narnia', 'Narnia', 'narnia');
  mint('/stuff/idea/Locality/cair-paravel', 'Cair Paravel', 'narnia/castle');
}

describe('TraceAddressController', () => {
  beforeEach(() => {
  ...
  it('renders the covering Locality, provenance, and the prefix chain', async () => {
    ...
    const ctrl = await StuffApi.create(() => new TraceAddressController());
