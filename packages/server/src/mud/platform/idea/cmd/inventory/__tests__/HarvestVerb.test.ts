...
const CROP_PATH = '/trade/farming/thing/crop/_test-carrot';
...
function tissue(): Material {
  return makeStuffAtPath(() => {
    ...
    return m;
  }, freshPath('/stuff/idea/material/_test/harv-tissue')) as unknown as Material;
}
...
function makePlant(opts: { harvestable?: boolean } = {}): Plant {
  return makeStuffAtPath(() => {
    ...
    return p;
  }, freshPath('/trade/farming/thing/plant/_harv'));
}
...
function makeBed(nitrogen = 100, water = 6): GardenBed {
  return makeStuffAtPath(() => {
    ...
    return bed;
  }, freshPath('/trade/farming/thing/bed/_harv'));
}
  ...
  beforeEach(() => {
    ...
    vi.spyOn(StuffApi, 'clone').mockImplementation((async (path: string) => {
      ...
      return makeStuffAtPath(() => {
        ...
        return c;
      }, `/trade/farming/thing/crop/_minted-${cropSeq}`);
