...
function mat(hardness: number, toughness: number, name: string): Material {
  ...
  m.setName(name);
  stampTemplatePathForTest(m, `/stuff/idea/material/test/gw-${seq++}`);
  return m;
}
...
function makeFighter(room: TestRoom): TestFighter {
  ...
  stampTemplatePathForTest(plan, `/stuff/idea/species/BodyPlan/test-gw-${id}`);

  const species = makeStuff(() => new Species());
  species.setBodyPlan(plan);
  stampTemplatePathForTest(species, `/stuff/idea/species/test/gw-${id}`);
