...
const AUTHOR = { getTemplatePath: () => '/platform/agent/Avatar/alice' } as unknown as Stuff;
...
describe('StudioLogic.scaffoldClass', () => {
  ...
  it('hands a non-wizard the reserved /home/<self>/drafts path (not persisted)', async () => {
    stubAuthorGateOpen(); // AUTHOR templatePath = /platform/agent/Avatar/alice
    ...
    expect(out.targetPath).toBe('/platform/thing/DraftThing.ts');
