...
const RUM_REL = 'stuff/idea/material/spirit/rum.yaml';
...
describe('dry-run', () => {
  it('reports the exact change set and writes nothing', async () => {
    ...
    contentRows().find((r) => r.path === '/stuff/idea/material/spirit/rum')!.data = { edited: true };
    writeDomainFile(root, { rel: 'stuff/idea/material/spirit/vodka.yaml' });
    recordOf('p')!.pins.push('/stuff/idea/material/spirit/rum');
    ...
    expect(
      ...
      'insert /stuff/idea/material/spirit/vodka',
      'pinned-skip /stuff/idea/material/spirit/rum',
      'update /stuff/idea/material/spirit/gin',
    ]);
...
describe('status', () => {
  it('joins manifests with records: conflicts, pins, failure', async () => {
    ...
    await PackApi.pin('p', '/stuff/idea/material/spirit/other');
    ...
    expect(s!.record!.pins).toEqual(['/stuff/idea/material/spirit/other']);
