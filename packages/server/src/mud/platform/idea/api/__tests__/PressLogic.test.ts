...
function installPublisher(): void {
  ...
  org.rosterSlots = [
    { positionKey: 'communications-director', assignee: '/platform/agent/Avatar/p1', schedule: [] },
    { positionKey: 'communications-director', assignee: '/platform/agent/Avatar/p2', schedule: [] },
  ];
...
describe('PressApi.publish (actor from context)', () => {
  it('derives the author from the execution context, not the request', async () => {
    ...
    expect(b.getAuthor()).toBe('/platform/agent/Avatar/p1');
    ...
    expect(found[0]!.getAuthor()).toBe('/platform/agent/Avatar/p1');
  ...
  it('defaults realm/kind when omitted', async () => {
    const author = makeStuffAtPath(() => new TestAuthor(), '/platform/agent/Avatar/p2');
    const b = await publishAs(author, () =>
