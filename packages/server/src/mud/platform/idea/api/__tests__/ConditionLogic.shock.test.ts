...
function steel(): Material {
  ...
  stampTemplatePathForTest(m, `/stuff/idea/material/test/steel-${seq++}`);
  return m;
}
...
function bodied(): Creature {
  ...
  stampTemplatePathForTest(plan, `/stuff/idea/species/BodyPlan/test-shock-${id}`);

  const species = makeStuff(() => new Species());
  species.setBodyPlan(plan);
  stampTemplatePathForTest(species, `/stuff/idea/species/test/shock-${id}`);
