...
describe('PackLogic — reconcile (fixture packs, stubbed class resolution)', () => {
  ...
  it('insert: fresh DB → stamped rows, listed in `inserted`', async () => {
    const root = writePack('p', [
      { rel: 'stuff/idea/material/spirit/gin.yaml', data: { name: 'gin' } },
      { rel: 'stuff/idea/material/element/iron.yaml', data: { name: 'iron' } },
    ]);
    ...
    expect(contentRows().find((row) => row.path === '/stuff/idea/material/spirit/gin')!.data).toEqual(
      { name: 'gin' },
    );
  });
  ...
  it('update: changed file overwrites; second run is a no-op', async () => {
    const root = writePack('p', [
      { rel: 'stuff/idea/material/spirit/gin.yaml', data: { name: 'gin', density: 940 } },
    ]);
    ...
    expect(r2!.updated).toEqual(['/stuff/idea/material/spirit/gin']);
  ...
  it('delete: a stamped row whose file vanished is removed', async () => {
    const root = writePack('p', [
      { rel: 'stuff/idea/material/spirit/gin.yaml' },
      { rel: 'stuff/idea/material/spirit/rum.yaml' },
    ]);
    await PackApi.install([root]);
    expect(contentRows()).toHaveLength(2);

    rmSync(join(root, 'content/stuff/idea/material/spirit/rum.yaml'));
    const [r2] = await PackApi.install([root]);
    expect(r2!.deleted).toEqual(['/stuff/idea/material/spirit/rum']);
    expect(contentRows().map((row) => row.path)).toEqual(['/stuff/idea/material/spirit/gin']);
  });
  ...
  it('an unstamped row at a pack path refuses the pack — untouched, nothing adopted', async () => {
    ...
    expect(r!.failure?.error).toMatch(/'\/stuff\/idea\/material\/spirit\/gin' but it exists with no sourcePack stamp/);
  ...
  it('requires-kernel: a bogus class aborts before any write', async () => {
    const root = writePack('p', [
      { rel: 'stuff/idea/material/spirit/gin.yaml' },
      { rel: 'stuff/idea/material/bad.yaml', class: '/stuff/idea/material/DoesNotExist' },
    ]);
...
describe('PackLogic — reconcile name banks (fixture packs, the name-bank document kind)', () => {
  ...
  it('insert: name-bank files → stamped name-bank documents (data.key = file basename)', async () => {
    const root = writePack(
      'p',
      [{ rel: HOMO, class: '/platform/idea/species/Species' }],
      [],
      [
...
describe('PackLogic — pack integration (real packs + real class resolution)', () => {
  it('install() discovers the shipped packs: base-library + species-and-names + arcana + newbie-wilds', async () => {
    ...
    const HOMO =
      '/stuff/idea/species/animalia/chordata/mammalia/primates/hominidae/homo';
    // The kingdom Clade, the canonical human, and the two new casts.
    expect(sp!.inserted).toContain('/stuff/idea/species/animalia');
