...
describe('ConvictionLogic.tally', () => {
  ...
  it('sums signed by net (yea − nay) and weights by conviction', async () => {
    ...
    await ConvictionApi.hold('/platform/agent/Avatar/yes', 'consumer', TARGET, { yea: 1, nay: 0 }, t0);
    await ConvictionApi.hold(
      '/platform/agent/Avatar/no',
      'consumer',
      TARGET,
      { yea: 0, nay: 1 },
      t0 + BUILD_MS / 2
  ...
  it('weights each holder by their own standing scalar', async () => {
    ...
    vi.spyOn(InfluenceApi, 'standingOf').mockImplementation((subject, stock) => {
      const scalar = subject === '/platform/agent/Avatar/whale' ? 100 : 1;
      return new InfluenceStanding(subject, stock, scalar, Band.fromScalar(scalar));
    });
    const t0 = 5_000_000;
    await ConvictionApi.hold('/platform/agent/Avatar/whale', 'consumer', TARGET, { yea: 1, nay: 0 }, t0);
    await ConvictionApi.hold('/platform/agent/Avatar/minnow', 'consumer', TARGET, { yea: 1, nay: 0 }, t0);
...
describe('ConvictionLogic abstain + quorum', () => {
  ...
  it('quorum counts present positions (abstain + directional), never non-voters', async () => {
    stubStanding(10);
    const t0 = 5_000_000;
    await ConvictionApi.abstain('/platform/agent/Avatar/abstainer', 'consumer', TARGET, t0);
    await ConvictionApi.hold('/platform/agent/Avatar/voter', 'consumer', TARGET, { yea: 1, nay: 0 }, t0);
