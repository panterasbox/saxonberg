...
describe('the recipe document kind', () => {
  const MARTINI = {
    ...
    outputTemplate: '/world/lounge/thing/cocktail-glass',
  };
...
describe('the blueprint document kind', () => {
  it('installs a curated blueprint at /platform/blueprints/<id>; a player-minted blueprint with the same id is refused, not clobbered', async () => {
    ...
    writeDocumentFile(packRoot, 'blueprints', 'coin', { name: 'Coin', kind: 'concrete', baseClass: 'Thing', classPath: '/stuff/thing/Coin' });
    const [r] = await PackApi.install([packRoot]);
    ...
    expect(rowsOfKind('blueprint')[0]).toMatchObject({
      ...
      owner: '/platform',
      sourcePack: 'platform',
      data: { blueprintId: 'coin', classPath: '/stuff/thing/Coin' },
    });
    ...
    expect(rowsOfKind('blueprint')[0]).toMatchObject({ _id: 'minted', owner: '/platform/agent/Avatar/x' });
  });
});
...
describe('the command-view document kind', () => {
  ...
  it('a fixture platform/cmd/perception/look.yaml lands at /platform/cmd/perception/look (no root join) with the view as data', async () => {
    const packRoot = writePack('platform', [], { root: '/platform' });
    writeDocumentFile(packRoot, 'platform/cmd/perception', 'look', LOOK);
    const [r] = await PackApi.install([packRoot]);
    expect(r!.failure).toBeNull();
    expect(r!.inserted).toEqual(['/platform/cmd/perception/look']);
