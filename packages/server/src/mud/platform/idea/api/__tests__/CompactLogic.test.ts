...
describe("CompactApi — the committee reads", () => {
  ...
  it("an organization-held parcel resolves to the organization arm", async () => {
    ...
    const committee = await CompactApi.committeeOf("/stuff/thing/gear/hat");
    expect(committee).toEqual({
      kind: "organization",
    ...
    await expect(CompactApi.committeeChannelOf("/stuff/thing/gear/hat")).resolves.toBeNull();
    await expect(CompactApi.ensureCommitteeChannel("/stuff/thing/gear/hat")).resolves.toBeNull();
  });
  ...
  it("isCommitteeMember over an organization is staff-or-head, resident only", async () => {
    ...
    vi.spyOn(EmploymentApi, "holdsAuthority").mockImplementation(
      async (s) => s?.getIdentityPath() === "/platform/agent/Avatar/pm",
    );
