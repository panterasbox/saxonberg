...
describe('the boot union', () => {
  it('unions two packs\' lists in install order, topo-sorts cross-pack dependsOn, reports counts by role', async () => {
    const platform = writePack('platform', [ROW('obj/EventRegistry.yaml'), ROW('platform/idea/GroupRegistry.yaml')], {
      manifest: { boot: [ENTRY('/platform/idea/EventRegistry', 'sync-read'), ENTRY('/platform/idea/GroupRegistry', 'sync-read')] },
    });
    const world = writePack('world-seed', [ROW('studio/lounge/terminal.yaml')], {
      dependsOn: ['platform'],
      manifest: { boot: [ENTRY('/studio/lounge/terminal', 'producer', ['/platform/idea/GroupRegistry'])] },
    });
    ...
    expect(recordOf('world-seed')!.boot).toEqual([ENTRY('/studio/lounge/terminal', 'producer', ['/platform/idea/GroupRegistry'])]);

    const union = await PackApi.bootManifest([platform, world]);
    expect(union).toEqual([
      { templatePath: '/platform/idea/EventRegistry', packId: 'platform', role: 'sync-read' },
      { templatePath: '/platform/idea/GroupRegistry', packId: 'platform', role: 'sync-read' },
      { templatePath: '/studio/lounge/terminal', packId: 'world-seed', role: 'producer', dependsOn: ['/platform/idea/GroupRegistry'] },
    ]);
