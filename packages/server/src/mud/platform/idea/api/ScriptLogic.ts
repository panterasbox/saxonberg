...
function resolveLimits(authorPath?: string): ResourceLimits {
  const platform =
    ...
    (authorPath.startsWith("/platform/") || authorPath.startsWith("/stuff/") || authorPath.startsWith("/world/") || authorPath.startsWith("/trade/"));
  return {
 ...
 * {@link ScriptApi}.
 *
 * Lives at `/platform/idea/api/script` (a stateless `Stuff` singleton, no backing
