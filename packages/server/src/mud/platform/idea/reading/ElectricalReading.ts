...
export default class ElectricalReading extends Reading {
  protected override async analyze(
    ...
    const model = { target: { stuff: subject, raw: param } } as unknown as AnalyzeElectricalModel;
    ...
    if (material) {
      ...
      lines.push(
        Mml.compose`  conductivity: ${sigma.tag()} (${sigma.formatMml(undefined, undefined, { channel: 'electrical' })})`,
      );
      // Its contact resistance (band + raw Ω).
      const r = MaterialApi.contactResistance(material);
      lines.push(Mml.compose`  contact resistance: ${r.tag()} (${r.formatMml(undefined, undefined, { channel: 'electrical' })})`);
      anything = true;
    }
    ...
    if (MixinApi.isEnergized(stuff)) {
      ...
      lines.push(Mml.compose`  potential: ${v.formatMml(undefined, undefined, { channel: 'electrical' })}`);
      anything = true;
    }
