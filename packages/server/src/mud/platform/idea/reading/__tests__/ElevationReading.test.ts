/**
 * ElevationReading celestial branch — `measure altitude
...
import ElevationReading from '../ElevationReading';
...
describe('ElevationReading (celestial branch)', () => {
  beforeEach(() => {
  ...
  it('reads the sun altitude / azimuth with a sextant', async () => {
    ...
    const reading = withRow(await StuffApi.create(() => new ElevationReading()), 'elevation');
    await driveMeasure(reading, makeContext(avatar, room), { subject: { stuff: null, raw: 'sun' }, tools: [await instrumentWith('sighting')] });
  ...
  it('reads the moon altitude / azimuth with a sextant', async () => {
    ...
    const reading = withRow(await StuffApi.create(() => new ElevationReading()), 'elevation');
    await driveMeasure(reading, makeContext(avatar, room), { subject: { stuff: null, raw: 'moon' }, tools: [await instrumentWith('sighting')] });
  ...
  it('refuses the sun/moon reading without a sextant', async () => {
    ...
    await driveMeasure(reading, ctx, { subject: { stuff: null, raw: 'sun' } });
