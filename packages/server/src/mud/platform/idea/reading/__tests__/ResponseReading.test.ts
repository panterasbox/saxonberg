...
import ResponseReading from '../ResponseReading';
...
describe('ResponseReading', () => {
  ...
  it('previews a weapon — delivered channels, and none for what it cannot', async () => {
    ...
    await driveAnalyze(reading, makeContext(me, loc), { subject: { stuff: dagger, raw: 'dagger' } });
  ...
  it('previews an armor piece — how it turns each channel', async () => {
    ...
    const reading = withRow(await StuffApi.create(() => new ResponseReading()), 'response');
    await driveAnalyze(reading, makeContext(me, loc), { subject: { stuff: plate, raw: 'breastplate' } });
    const frame = me.received[0] as { body: string };
  ...
  it('rejects a target with no construction', async () => {
    ...
    const reading = withRow(await StuffApi.create(() => new ResponseReading()), 'response');
    await driveAnalyze(reading, makeContext(me, loc), { subject: { stuff: rock, raw: 'rock' } });
    const frame = me.received[0] as { body: string };
