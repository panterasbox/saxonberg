/**
 * WeatherReading — `analyze weather [<location>]` renders the
...
async function runVerb(
  ...
  const reading = withRow(await StuffApi.create(() => new WeatherReading()), 'weather');
  const ctx = makeContext(avatar, room);
  await driveAnalyze(reading, ctx, { subject: here(room) });
