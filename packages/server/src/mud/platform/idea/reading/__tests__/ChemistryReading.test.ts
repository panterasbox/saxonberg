...
describe('ChemistryReading', () => {
  ...
  it('emits chemistry data with canonical Quantity markup', async () => {
    ...
    const reading = withRow(await StuffApi.create(() => new ChemistryReading()), 'chemistry');
    await driveAnalyze(reading, makeContext(avatar, room), { subject: { stuff: sword, raw: 'sword' } });
  ...
  it('rejects a non-Tangible target', async () => {
    ...
    const reading = withRow(await StuffApi.create(() => new ChemistryReading()), 'chemistry');
    await driveAnalyze(reading, makeContext(avatar, room), { subject: { stuff: idea, raw: 'thought' } });
  });
