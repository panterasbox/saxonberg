 ...
 * cheap; the developer/pedagogical companion to `trace atmosphere` /
 * `trace address`. The Barometer separately reads the
...
export default class WeatherReading extends Reading {
  protected override async analyze(
    ...
    const model = { location: { stuff: subject, raw: param } } as unknown as AnalyzeWeatherModel;
