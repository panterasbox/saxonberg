 ...
 * and behavior. Lives at `/platform/idea/AccessRegistry`, sibling to
...
const AccessApiCallers = SecurityPolicies.AnyOf(
  ...
  SecurityPolicies.FromTemplate('/platform/idea/api/access'),
);
...
export default class AccessRegistry extends AccessRegistryBase {
  ...
  public async setWizardMembership(
    ...
    // membership keys uniformly on `/platform/agent/Avatar/<playerId>`.
   ...
   * up to `/stuff/idea/lounge` (match, extant FolderZone) → returns it.
   ...
   * path (a player as `/platform/agent/Avatar/<id>`, an NPC as its own path). Null
   ...
   * `/platform/agent/Avatar/<id>` it acts as, so a wizard is still a wizard inside
   ...
   * Avatar's own `templatePath` (`/platform/agent/Avatar/<key>`, a transferred title)
