 ...
 *
 * A singleton `Idea` at `/platform/idea/WikiRegistry`, the shipped
...
const WikiControllerOnly = SecurityPolicies.FromModule(
  '/platform/idea/cmd/system/WikiController',
);
...
const WikiWriters = SecurityPolicies.AnyOf(
  WikiControllerOnly,
  SecurityPolicies.FromModule('/platform/idea/api/PackLogic#PackLogic'),
);
...
export default class WikiRegistry extends WikiRegistryBase {
   ...
   * throwaway whose identity (`/platform/agent/Avatar/guest-<uuid>`) evaporates
