  ...
  describe('pathFor — unit encoding', () => {
    it('plain units pass through', () => {
      expect(QuantityMarshaller.pathFor('kg')).toBe(
        '/platform/idea/persistence/QuantityMarshaller/kg'
      );
      expect(QuantityMarshaller.pathFor('K')).toBe(
        '/platform/idea/persistence/QuantityMarshaller/K'
      );
    });

    it('encodes "/" as "-per-"', () => {
      expect(QuantityMarshaller.pathFor('g/mol')).toBe(
        '/platform/idea/persistence/QuantityMarshaller/g-per-mol'
      );
    });

    it('encodes "³" as "3"', () => {
      expect(QuantityMarshaller.pathFor('kg/m³')).toBe(
        '/platform/idea/persistence/QuantityMarshaller/kg-per-m3'
      );
    });
  });
