...
describe('Topic', () => {
  ...
  it('participates in findByTemplatePath via path stamping', () => {
    const t = makeStuffAtPath(
      () => new Topic(),
      '/platform/idea/Topic/speech.vocal',
    );
    expect(
      StuffApi.findByTemplatePath('/platform/idea/Topic/speech.vocal'),
    ).toBe(t);
  });

  it('declares TEMPLATE_PATH_PREFIX = /platform/idea/Topic/', () => {
    expect(Topic.TEMPLATE_PATH_PREFIX).toBe('/platform/idea/Topic/');
  });
});
