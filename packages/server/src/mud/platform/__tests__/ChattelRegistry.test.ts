...
describe("chattel possession core", () => {
  ...
  it("transfer re-stamps; ownerOf follows the new owner", async () => {
    ...
    expect(events[1]?.from).toEqual({ kind: "player", templatePath: "/platform/agent/Avatar/alice" });
    expect(events[1]?.to).toEqual({ kind: "player", templatePath: "/platform/agent/Avatar/bob" });
  });
  ...
  it("ownership survives a persistence round-trip — now OWNER-side", async () => {
    ...
    const reborn = makeStuffAtPath(() => new EstateHolder(), "/platform/agent/Avatar/alice");
  ...
  it("an unstamped good resolves to its author (stamp ?? authorOf)", async () => {
    ...
    col("authoring_events").push({
      _id: "ae1",
      path: TORCH_PATH,
      author: "/platform/agent/Avatar/carol",
      kind: "save",
      at: 100,
      realAt: 100,
    });
