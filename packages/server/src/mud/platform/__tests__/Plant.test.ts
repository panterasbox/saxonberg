...
function tissue(): Material {
  ...
  return makeStuffAtPath(() => {
    ...
    return m;
  }, `/stuff/idea/material/_test/tissue-${matSeq}`) as unknown as Material;
}
...
function makePlant(
  ...
  plantSeq += 1;
  const path = `/trade/produce/thing/plant/_test-${plantSeq}`;
  return makeStuffAtPath(() => {
    const p = new Plant();
...
function makePot(soil: number, capacity = soil): PlantPot {
  ...
  return makeStuffAtPath(() => {
    ...
    return pot;
  }, `/trade/produce/thing/pot/_test-${potSeq}`);
}
...
describe('Plant — the growing object', () => {
  ...
  it('flowering sets exactly one seed, into the POT (a plant is no container)', async () => {
    ...
    const cloneSpy = vi
      ...
      .mockImplementation((async (path: string) => {
        ...
        return makeStuffAtPath(() => {
          ...
          s.setGrowsIntoPath('/trade/produce/thing/plant/_seedtest');
          return s;
        }, `${path}#${minted}`);
    ...
    try {
      const room = makeStuff(() => new LitRoom());
      room.setAmbientFlux(40);
      const p = makePlant(lilyProfile(), { seedPath: '/trade/produce/thing/seed/_test' });
      const big = makePot(3);
      ContainmentApi.move(big, room);
      pot(p, big);
      ...
      expect(p.getSeedTemplatePath()).toBe('/trade/produce/thing/seed/_test');
  ...
  it('two plants cloned from the same template hold independent state', () => {
    ...
    const a = makeStuffAtPath(() => {
      ...
      return p;
    }, '/trade/produce/thing/plant/shared');
...
describe('PlantPot + Seed — the rest of the assembly', () => {
  ...
  it('a seed names the plant it grows into and is discrete, never a stack', () => {
    const s = makeStuffAtPath(() => {
      ...
      seed.setGrowsIntoPath('/trade/produce/thing/plant/peace-lily');
      return seed;
    }, '/trade/produce/thing/seed/peace-lily');
    expect(s.getGrowsIntoPath()).toBe('/trade/produce/thing/plant/peace-lily');
