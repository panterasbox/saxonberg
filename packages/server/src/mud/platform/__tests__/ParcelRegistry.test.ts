...
async function addGroupMember(
  ...
  // `/platform/agent/Avatar/<playerId>`), matching how `makeAvatar` places the avatar.
  group!.addMember(`/platform/agent/Avatar/${playerId}`, role);
  await group!.save();
...
describe("ParcelApi.subdivide", () => {
  ...
  it("mints a child row (inherited owner + parentParcel) and a genesis event", async () => {
    ...
    expect(events[0]!.actor).toBe("/platform/agent/Avatar/dave");
  });
});
...
describe("ParcelRegistry — the narrow-entry gate", () => {
  ...
  it("denies a caller outside the parcel subsystem", async () => {
    await boot();
    const reg = StuffApi.findByTemplatePath<ParcelRegistry>("/platform/idea/ParcelRegistry");
...
describe("ParcelApi — the lease (use-grant) surface", () => {
  ...
  it("grant → hasUseGrant true; heldUnitOf returns it; another holder false", async () => {
    ...
    expect(await ParcelApi.grantUse("/dorms/f1-r1", "/platform/agent/Avatar/iris", null)).toBe(true);
    expect(await ParcelApi.hasUseGrant("/dorms/f1-r1", "/platform/agent/Avatar/iris")).toBe(true);
    expect(await ParcelApi.hasUseGrant("/dorms/f1-r1", "/platform/agent/Avatar/bob")).toBe(false);

    const held = await ParcelApi.heldUnitOf("/platform/agent/Avatar/iris");
    expect(held?.getExtent()).toBe("/dorms/f1-r1");
    expect(await ParcelApi.heldUnitOf("/platform/agent/Avatar/bob")).toBeNull();
  });
  ...
  it("a past-expiry grant is inactive; revoke clears it", async () => {
    ...
    await ParcelApi.grantUse("/dorms/f1-r2", "/platform/agent/Avatar/iris", Date.now() - 1000);
    expect(await ParcelApi.hasUseGrant("/dorms/f1-r2", "/platform/agent/Avatar/iris")).toBe(false);

    // A fresh indefinite grant, then revoke.
    await ParcelApi.grantUse("/dorms/f1-r2", "/platform/agent/Avatar/iris", null);
    expect(await ParcelApi.hasUseGrant("/dorms/f1-r2", "/platform/agent/Avatar/iris")).toBe(true);
    expect(await ParcelApi.revokeUse("/dorms/f1-r2", "/platform/agent/Avatar/iris")).toBe(true);
    expect(await ParcelApi.hasUseGrant("/dorms/f1-r2", "/platform/agent/Avatar/iris")).toBe(false);
  });
  ...
  it("grant persists through save / findByExtent", async () => {
    ...
    await boot();
    await ParcelApi.grantUse("/dorms/f1-r3", "/platform/agent/Avatar/iris", null);

    const row = await ParcelRecord.findByExtent("/dorms/f1-r3");
    expect(row?.getGrants()).toHaveLength(1);
    expect(row?.getGrants()[0]?.holder).toBe("/platform/agent/Avatar/iris");
    expect(row?.getGrants()[0]?.kind).toBe("lease");
  });

  it("grantUse returns false when no parcel claims the extent", async () => {
    await boot();
    expect(await ParcelApi.grantUse("/nope", "/platform/agent/Avatar/iris", null)).toBe(false);
  });
});
...
describe("AccessApi — can / canMutateZone over a parcel-owned slice (regression)", () => {
  ...
  it("a player-owned parcel authorizes exactly that individual (transferred title)", async () => {
    seedParcel({
      extent: "/plot/17",
      owner: { kind: "player", templatePath: "/platform/agent/Avatar/alice" },
    });
    await bootWithAccess();
...
describe("ParcelApi.landUseOf — longest-prefix zoning", () => {
  ...
  it("⭐ a path-branch row with no use answers wild — so cultivation is refused", async () => {
    // /studio and /stuff/idea/lounge are titles over the TEMPLATE TREE, not
    ...
    expect(ParcelApi.landUseOf("/stuff/idea/lounge")).toBe("wild");
    expect(ParcelApi.cultivationScaleAt("/studio")).toBe("none");
    expect(ParcelApi.cultivationScaleAt("/stuff/idea/lounge")).toBe("none");
  });
