...
describe('archived rows are invisible, never reaped', () => {
  it('ChannelCatalogue.resolveByName skips an archived channel', async () => {
    ...
    const cat = makeStuffAtPath(() => new ChannelCatalogue(), '/platform/idea/ChannelCatalogue');
    await cat.warmCache();
  ...
  it('SubjectCatalogue.resolveByTitle skips an archived subject', async () => {
    ...
    const cat = makeStuffAtPath(() => new SubjectCatalogue(), '/platform/idea/SubjectCatalogue');
    await cat.warmCache();
