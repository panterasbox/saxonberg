...
import WikiRegistry, { WikiDenied, WikiConflict } from '../idea/WikiRegistry';
import WikiNamespaceZone from '../idea/WikiNamespaceZone';
...
describe('subject binding (5, 6)', () => {
  ...
  it('the reverse lookup is TOTAL — an answer for every subject (6)', async () => {
    ...
    expect(
      (await registry.articleFor({ kind: 'template', ref: '/stuff/idea/material/oak' }))
        ?.getSlug(),
    ).toBe('oak');
    // Undocumented is `null`, not an absence the caller has to handle
    // differently.
    expect(
      await registry.articleFor({ kind: 'template', ref: '/stuff/idea/material/pine' }),
    ).toBeNull();
  });
...
describe('⭐ no game model is touched (7)', () => {
  ...
  it('a page names its subject; the subject knows nothing of the page', async () => {
    ...
    expect(page.getSubject()?.ref).toBe('/stuff/idea/material/oak');
...
describe('the installer as a writer (content-packs wave 2, D9)', () => {
  ...
  it('⚠ the controller never passes asInstaller — the option is the installer’s alone', () => {
    const src = readFileSync(
      fileURLToPath(new URL('../idea/cmd/system/WikiController.ts', import.meta.url)),
      'utf8',
    );
