 ...
 * `/world/terminus/registry` to the `terminus` group, and
...
function seededRegistry(): BusinessEntity {
  const seed = parse(
    readFileSync(
      fileURLToPath(
        new URL(
          '../../../../../../content/world-seed/content/world/terminus/registry/business.yaml',
          import.meta.url,
        ),
      ),
      'utf8',
...
function stubTitle(): void {
  vi.spyOn(ParcelApi, 'ownerOf').mockImplementation(
    async (path: string): Promise<ParcelOwner | null> =>
      path.startsWith('/world/terminus/registry')
...
describe('the Registry, as seeded', () => {
  ...
  it('⭐ resolves its authority to the city committee, and gates on it', async () => {
    ...
    expect(biz.getAppointingAuthority()).toEqual({
      kind: 'committee',
      parcel: '/world/terminus/registry',
    });
