...
const PRODUCE = `${PACKS}trade-produce/content/`;
...
function seedDomain(): void {
  ...
  addSeed('/trade/produce/thing/pot/starter', `${PRODUCE}trade/produce/thing/pot/starter.yaml`);
  addSeed('/trade/produce/thing/pot/small', `${PRODUCE}trade/produce/thing/pot/small.yaml`);
  addSeed('/trade/produce/thing/pot/large', `${PRODUCE}trade/produce/thing/pot/large.yaml`);
  addSeed('/trade/produce/thing/plant/peace-lily', `${PRODUCE}trade/produce/thing/plant/peace-lily.yaml`);
  addSeed('/trade/produce/thing/plant/snake-plant', `${PRODUCE}trade/produce/thing/plant/snake-plant.yaml`);
  addSeed('/trade/produce/thing/seed/peace-lily', `${PRODUCE}trade/produce/thing/seed/peace-lily.yaml`);
  addSeed('/trade/produce/thing/seed/snake-plant', `${PRODUCE}trade/produce/thing/seed/snake-plant.yaml`);
...
describe('the dorm houseplant — content and placement', () => {
  ...
  it('⭐ the acceptance walk: soil → plant → tend → flower → outgrow → repot', async () => {
    ...
    const small = await StuffApi.clone<PlantPot>('/trade/produce/thing/pot/small');
    const seed = await StuffApi.clone<Seed>('/trade/produce/thing/seed/snake-plant');
    ...
    const large = await StuffApi.clone<PlantPot>('/trade/produce/thing/pot/large');
    ...
    expect(setSeed.getGrowsIntoPath()).toBe('/trade/produce/thing/plant/snake-plant');
  });
});
...
describe('the dorm houseplant — durability', () => {
  ...
  it('⭐ carry it out of the dorm and it stays yours — its own record, no dorm in the path', async () => {
    ...
    const own = records.find(
      (r) => r.scope === '/trade/produce/thing/plant/peace-lily' && r.owner === key,
    );
    ...
    expect(refs).toContain('/trade/produce/thing/plant/peace-lily');
  ...
  it('⭐ the restart-rollback property: the elapsed time survives, the watering needs a capture', async () => {
    ...
    const restored = makeStuffAtPath(
      () => new Plant(),
      '/trade/produce/thing/plant/peace-lily',
    );
