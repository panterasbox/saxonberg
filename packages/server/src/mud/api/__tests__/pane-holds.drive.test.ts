/**
 * Pane lifetime, **driven** — not hand-refreshed.
 *
 * ⭐⭐ **This is the `pane-holds-need-their-wake` lesson, written as a
 * suite.** Eleven green tests once missed an immortal pane because
 ...
 * one: *a pane held by `present` disappears when the subject leaves,
...
import type { PaneReleaseReason } from '@saxonberg/types';
...
function releases(h: Harness): Array<[string, PaneReleaseReason]> {
  return h.envelopes
    .filter((e) => e.type === 'mql-subscription-released')
    .map((e) => [e.subscriptionId!, e.reason as PaneReleaseReason]);
}
...
beforeEach(() => {
  ...
  // leaking into the next case makes an `unanswered` pane look immortal
...
describe('⭐ the pair the requirement states', () => {
  it('`present` releases when they leave; `unanswered` does not', async () => {
    ...
    // ⚠ Stubbed BEFORE the subscribe. A pane's hold is evaluated at
    // open as well as on every re-resolve — a pane that opens
    ...
    // ⭐ …and the question is still owed, so its pane is still open.
    expect(seen.map(([id]) => id)).not.toContain('a-question');
  });

  it('⭐⭐ the `unanswered` pane goes when the question is ANSWERED', async () => {
    const h = await setup();
    const bob = await StuffApi.create(() => new Avatar());
    bob.setName('Bob');
    ContainmentApi.move(bob, h.here);
     ...
     * when a prompt settled. The pane was **immortal**: the player
...
describe('⭐ a pinned pane does not leave at all', () => {
  it('survives the release of its hold condition', async () => {
    ...
    MqlSubscriptionApi.setPanePinned(h.interactive, 'about-bob', true);
    await settle();
  ...
  it('a dismiss drops one whose condition still holds, with a reason', async () => {
    ...
    // keep a pane, never dismiss one.
    MqlSubscriptionApi.setPanePinned(h.interactive, 'about-bob', false);
    await settle();
...
describe('the pin answers on the wire', () => {
  ...
  it('resolves a pane by its CATALOGUE name, not just its handle', async () => {
    const h = await setup();
    const bob = await StuffApi.create(() => new Avatar());
    bob.setName('Bob');
    ContainmentApi.move(bob, h.here);
    ...
    // `cockpit pane list` prints the catalogue id, so `cockpit pane pin
    // place` is the form the verb's own help shows. Looking up only the
    // handle answered "no open pane 'place'" — a surface that names
...
describe('a subject pane', () => {
  ...
  it('opens about the named Stuff, and the server owns the query', async () => {
    ...
    const result = h.envelopes.find(
      (e) => e.type === 'mql-subscription-result',
    ) as { result?: Array<{ stuffId?: string }>; pane?: string } | undefined;
    expect(result?.result?.[0]?.stuffId).toBe(bob.stuffId);
    // ⭐ The catalogue name is echoed so a server-pushed pane tells the
    // client which body to draw for a handle it never minted.
    expect(result?.pane).toBe('agent');
  });
  ...
  it('⚠⚠ resolves nothing for a subject the viewer cannot PERCEIVE', async () => {
     ...
     * perception gate, because `#id` is authoring-tier tooling. A pane
