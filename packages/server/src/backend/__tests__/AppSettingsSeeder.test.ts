...
describe("AppSettingsSeeder", () => {
  ...
  it("seeds the YAML values into a fresh app_settings row", async () => {
    ...
    expect(added).toBe(287); // 146 base + 7 Attendant+Goodkin + 19 storms-and-wetness + 17 concealment/detection/hazard/movement + 12 stealth-deployables + 2 residency reset + 2 retail consignment + 15 fire (3 heat-channel + 4 combustion + 3 tick/spread + 5 chemistry) + 20 magic (3 cast + 7 faculty pool/recovery + 4 composure + 1 potency + 2 overchannel + 3 effect magnitudes) + 3 combat formations + 4 work-contracts (banking.defaultCustodianBank + 3 contract.*) + 4 combat-hooks (3 influence + combat.natural.largeBodyMassKg) + 4 kick relay (replayWindowSec + dedupTtlSec + dedupMaxSize + resolveCacheTtlMs) + 11 crafting (broken + delivery floors, wear-per-use, 3 keenness, repair pricing/heat, salvageRate) + 13 husbandry (step/maxSteps, vigor tau, goodAt, deathAt, rootFloor, 3 bands, 2 warmth, 2 soil nutrient, 4 grade ladder) + 2 land (lot price + area) + 2 sandbox (sweeper.intervalMs + session.graceMs)
    ...
    pm.setFindResult([
      {
        ...
        values: {
          ...
          [AppSettingKeys.landLotPriceMinor]: "4000",
          [AppSettingKeys.landLotAreaM2]: "1000",
  ...
  it("merges a missing key while preserving an operator-changed one", async () => {
    ...
    expect(added).toBe(286); // 287 total − 1 operator-preset key
    expect(pm.saves).toHaveLength(1);
    const values = savedValues(pm);
