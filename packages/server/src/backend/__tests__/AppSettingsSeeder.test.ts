...
describe("AppSettingsSeeder", () => {
  ...
  it("seeds the YAML values into a fresh app_settings row", async () => {
    ...
    expect(added).toBe(240); // 146 base + 7 Attendant+Goodkin + 19 storms-and-wetness + 17 concealment/detection/hazard/movement + 12 stealth-deployables + 2 residency reset + 2 retail consignment + 15 fire (3 heat-channel + 4 combustion + 3 tick/spread + 5 chemistry) + 20 magic (3 cast + 7 faculty pool/recovery + 4 composure + 1 potency + 2 overchannel + 3 effect magnitudes)
  ...
  it("merges a missing key while preserving an operator-changed one", async () => {
    ...
    expect(added).toBe(239); // 240 total − 1 operator-preset key
    expect(pm.saves).toHaveLength(1);
    const values = savedValues(pm);
