...
describe("AppSettingsSeeder", () => {
  ...
  it("merges a missing key while preserving an operator-changed one", async () => {
    ...
    // + 2 banking + 3 youtube (historyCap, reconnectBackoffMs,
    // pollIntervalMs) + 3 social (social.baselineRules, social.defaultColor,
    // social.idleAfter) + 4 bulletin (tickerWindow, maxPins,
    // headlineMaxLength, bodyMaxLength) + 3 residency (eviction.mode,
