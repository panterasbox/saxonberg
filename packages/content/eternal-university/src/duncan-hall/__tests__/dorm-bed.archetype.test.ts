...
const SEEDS = fileURLToPath(new URL("../../../../eternal-university/content/", import.meta.url));
// The owned bed archetype is the generic-objects pack's row (content-packs wave 3).
const OBJECTS = fileURLToPath(new URL("../../../../generic-objects/content/", import.meta.url));
...
describe("the dorm bed against the bedroom archetype", () => {
  it("the DORM bed is lieable too — the mechanic reaches the room players have", () => {
    ...
    const dorm = read("world/terminus/eternal/duncan-hall/thing/bed.yaml");
  ...
  it("the dorm retrofit changed NO class: path — no live-record migration", () => {
    ...
    const dorm = read("world/terminus/eternal/duncan-hall/thing/bed.yaml");
    expect(dorm.class).toBe("/world/terminus/eternal/duncan-hall/thing/Bed");
  });
});
