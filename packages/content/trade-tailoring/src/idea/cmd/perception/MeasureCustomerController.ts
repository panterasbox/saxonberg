/**
 * MeasureCustomerController — `measure <customer>`, and ⭐ **the fitting
 * is a SCENE.**
 *
 ...
 * falls out of choosing the attendant shape for the shop. Measuring
 * somebody who is not your customer is simply not something the verb
 * offers.
 *
...
export default class MeasureCustomerController extends CommandController<MeasureModel> {
  execute(model: MeasureModel, context: CommandContext): void {
    const giver = context.commandGiver;
    const subject = model.subject?.stuff ?? null;
    if (!subject) {
      this.decline(context, Mml.compose`Measure whom?`, 'no-subject');
      return;
    }
