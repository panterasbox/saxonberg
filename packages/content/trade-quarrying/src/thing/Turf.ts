 ...
 *
 * `CuredMixin(Firewood)`. A turf comes out of a bog nearly all water, and it
 ...
 *
 * ⭐ And what dries it is the weather. `CuredMixin`'s two-way arm runs against
 ...
 *
 * ⚠⚠ **Cured WITHOUT Freshness, which is the anticipated case made real.**
 ...
 * load in order to express drying"* — and until now every `CuredMixin` host in
...
import { CuredMixin } from '@saxonberg/server/mud/lib/material/Cured';

export default class Turf extends CuredMixin(Firewood) {}
