/**
 * SurveyReading — the shared base for the geological channels
 * (`strike`, `dip`, `ground`, and Stage B's `grade`).
 *
 * ## ⭐⭐ Why these live in the PACK, and what changed
 *
 * They used to be controllers NAMED from the platform's own
 * `measure`/`analyze` views — which put a mining stanza in a kernel file,
 * advertised `measure strike` in an install with no mining pack, and
 * died on dispatch there with a `controller-error` note.
 *
 * They are `Reading` rows now. The pack ships
 * `content/trade/mining/idea/reading/strike.yaml` and the class beside
 * it, and **nothing in the platform names either**: install the pack,
 * get the channel; remove it, and the channel is simply not one. The
 * home was always right and the mechanism was wrong.
 *
...
export abstract class SurveyReading extends Reading {
  ...
  protected async geologyBandOf(giver: Stuff): Promise<{ band: CompetenceBandName; errorDeg: number; solveFrom: number }> {
    // ⭐ The band read itself is the LADDER's, not this file's — three
    // controllers held byte-identical copies of it before `Reading`
    // existed. What stays here is what is genuinely mining's: the
    // degrees of error a band buys, and how many points it can solve a
    // plane from.
