/**
 * SoilReading — the shared base for the three soil reads
...
export abstract class SoilReading extends Reading {
...
/**
  ...
  protected async soilBandOf(
    giver: Stuff,
  ): Promise<{ band: CompetenceBandName; phError: number; generaliseFrom: number }> {
    // ⭐ The band read is the LADDER's one copy now; what stays here is
    // what is genuinely farming's — the pH error a band buys, and how
    // many spots it takes before a reader will generalize to a field.
