...
import { SoilReading, READING_TOPIC, SOIL_SCIENCE } from '../../lib/SoilReading';
...
export default class SoilSurveyReading extends SoilReading {
  protected override async analyze(
    ...
    const { band, phError, generaliseFrom } = await this.soilBandOf(giver);
    const readings = this.recallReadings(giver, ground);
