...
import { SoilReading, READING_TOPIC, SOIL_SCIENCE, SOIL_TESTING } from '../../lib/SoilReading';
...
export default class AcidityReading extends SoilReading {
  protected override async measure(
    ...
    const { phError } = await this.soilBandOf(giver);
    ...
    this.rememberReading(giver, ground, spot, 'ph', sample.nativePh.toFixed(1));

    MessageApi.scene(giver)
      .topic(READING_TOPIC)
      .toSelf(
        Mml.compose`You work a pinch of the soil into ${Mml.thing(instrument)} and read it off: pH ${sample.nativePh.toFixed(1)} ± ${phError.toFixed(1)} — ${phPhrase(sample.nativePh)}`,
      )
      .toPeers(
