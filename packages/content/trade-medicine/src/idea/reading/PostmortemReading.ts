...
export default class PostmortemReading extends Reading {
  protected override async analyze(
    ...
    const model = { target: { stuff: subject, raw: param } } as unknown as PostmortemModel;
    ...
    if (!MixinApi.isOrganism(body) || !body.isDead()) {
      return this.decline(context, Mml.compose`They are still alive. Try ${'`analyze patient`'}.`, 'no-subject',
      );
    }

    const readability = body.getForensicReadability();
    if (readability <= 0) {
      return this.decline(context, Mml.compose`There is nothing left in them to read.`, 'no-subject',
      );
    }
