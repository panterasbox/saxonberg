...
import PlantController from '@saxonberg/server/mud/platform/idea/cmd/inventory/PlantController';
import WaterController from '@saxonberg/server/mud/platform/idea/cmd/bulk/WaterController';
import FeedController from '@saxonberg/server/mud/platform/idea/cmd/bulk/FeedController';
import HarvestController from '@saxonberg/server/mud/platform/idea/cmd/inventory/HarvestController';
import WateringCan from '@saxonberg/server/mud/platform/thing/WateringCan';
...
import { AmbientLitMixin } from '@saxonberg/server/mud/lib/perception/AmbientLit';
...
import { buildAllModalities } from '@saxonberg/server/mud/lib/perception/modalities/__tests__/test-helpers';
...
const YARD = '/world/terminus/hinkley-hills/yard';
const REGISTRY = '/world/terminus/registry';
const OFFICE = '/world/terminus/registry/office';
...
function makeBed(): GardenBed {
  return makeStuffAtPath(() => {
    ...
    return bed;
  }, fresh('/trade/farming/thing/bed/_hh'));
}
...
function makePot(): PlantPot {
  return makeStuffAtPath(() => {
    ...
    return pot;
  }, fresh('/trade/farming/thing/pot/_hh'));
}
...
function makeSeed(): Seed {
  return makeStuffAtPath(() => {
    ...
    s.setGrowsIntoPath('/trade/farming/thing/plant/carrot');
    return s;
  }, fresh('/trade/farming/thing/seed/_hh'));
}
...
describe('Hinkley Hills — the land-use gate', () => {
  beforeEach(async () => {
    ...
    vi.spyOn(StuffApi, 'clone').mockImplementation((async () => {
      ...
      return makeStuffAtPath(() => {
        ...
        return p;
      }, `/trade/farming/thing/plant/_hh-minted-${plantSeq}`);
  ...
  it('⭐ THE HERMIT TEST: unparcelled ground is not policed', async () => {
    ...
    const forest = roomAt(fresh('/world/wilderness/clearing'));
  ...
  it('⭐ a KEYED room resolves to its own lot, not to the district', async () => {
    ...
    const shared = fresh('/world/terminus/hinkley-hills/yard');
...
describe('⭐ the acceptance walk: plant → tend → harvest → feed → again', () => {
  beforeEach(async () => {
    ...
    vi.spyOn(StuffApi, 'clone').mockImplementation((async (path: string) => {
      plantSeq += 1;
      if (path.startsWith('/trade/farming/thing/crop/')) {
        return makeStuffAtPath(() => {
          const c = new Crop();
          c.setShortDescription('a bunch of carrots');
          return c;
        }, `/trade/farming/thing/crop/_walk-${plantSeq}`);
      }
      return makeStuffAtPath(() => {
        const p = new Plant();
        p.setShortDescription('a row of carrots');
        ...
        p.setHarvestTemplatePath('/trade/farming/thing/crop/carrot');
        p.setNutrientDraw(15);
        return p;
      }, `/trade/farming/thing/plant/_walk-${plantSeq}`);
    }) as unknown as typeof StuffApi.clone);
  });
