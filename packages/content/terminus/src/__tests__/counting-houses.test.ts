...
import { PropertiedMixin } from "@saxonberg/server/mud/lib/stuff/Propertied";
...
function seedData(rel: string): Record<string, unknown> {
  const file = ROOTS.map((r) => `${r}/${rel}`).find((f) => existsSync(f));
  if (!file) throw new Error(`no locality pack ships ${rel}`);
  const parsed = YAML.parse(readFileSync(file, "utf-8")) as {
...
describe("Goodkin seeds — the authored branch is well-formed", () => {
  it("the room exit chain walks crossing → avenue → hall ↔ parlor", () => {
    ...
    expect(hall.exits.north?.destination).toBe(
      "/world/terminus/counting-houses/circle-parlor",
    );
