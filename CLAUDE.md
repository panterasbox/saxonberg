...
- **Command YAML views**: in `mud/cmd/<category>/`, lowercase
  (`perception/look.yaml`, `social/say.yaml`). Functional categories:
  ...
  medical. **`mud/cmd/` and `mud/obj/command/` are CORE command trees —
  nothing content-specific belongs anywhere near them.** A verb that
  hardcodes one piece of content (e.g. `provision`/`unprovision`/`remodel`
  hardcode Duncan Hall's `DormWarren`) lives in that content's OWN namespace
  under `domain/`: the view in `domain/<content>/cmd/<verb>.yaml`, the
  controller in `domain/<content>/command/<Verb>Controller.ts`, its seed in
  `seeds/domain/<content>/command/<Verb>Controller.yaml` (e.g.
  `domain/eternal/duncan-hall/`). Content-owned verbs are referenced by
  their **mud-rooted absolute path** — the view's `controller:` field and a
  `commandContributions` entry both use `/domain/…/command/XController` and
  `/domain/…/cmd/x.yaml` (the leading `/` is what routes them past the core
  `cmd/` / `obj/command/` dirs; see `CommandLogic.discoverContentCommands`
  and `resolveControllerPath` in `CommandGiver`). Content commands are also
  afforded by content (the owning NPC/fixture's `commandContributions`),
  never by a core mixin. Only content-agnostic verbs (any venue can `craft`,
  `pay`) earn a core functional category.
- **Core command controllers**: in `mud/obj/command/<category>/`, e.g.
  `perception/LookController.ts`, `movement/GoController.ts` (content
  controllers live under `domain/<content>/command/`, above).
