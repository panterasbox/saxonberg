...
- Subsystem references in `docs/subsystems/`. Each doc is the source
  ...
  - [party.md](./docs/subsystems/party.md) — the **party operational core** (the party half of multi-party combat): a `Party` **Document** (`parties` collection, the `Group` precedent + combat/leadership fields) that **owns its own membership** and registers a fourth `party:<id>` **`GroupProvider`** — deliberately **NOT** a managed `Group` (reaching for `GroupApi` to *store* membership is the corrected anti-pattern; chat rides a `Channel` bound to `party:<id>` via `ChatApi.createBoundChannel`, no managed group minted). Two lifetimes over one primitive (**ad-hoc** in-memory / **durable** persisted + boot-warmed + dormant-not-destroyed, `muster`/`standdown`); one-active-party (`activePartyId`) but many rosters. `PartyMemberMixin` (sparse, `ApiOnly` setters) on **`Avatar`** + the hireable **`Mercenary`** (`= PartyMemberMixin(NPC)`), never the base `Character` (a plain NPC is `solo`). `PartyRegistry` (boot singleton, deps `GroupRegistry`) = the **sync** active-party map combat reads + the provider registration + durable warm. The **combat seam** = exactly two pure gated statics `PartyApi.sideOf` (three-rung **party → owner[pet, seam] → `solo:<templatePath>`** chain, NEVER null) / `areAllied`, frozen per-node at open/join; combat imports only these (one-way dep). `PartyApi`/`PartyLogic` (`/obj/api/party`) + the `party` verb (`form`/`invite`/`accept`/`enlist`/`leave`/`kick`/`disband`/`transfer`/`side`/`muster`/`standdown`/`list`/`show`). See [combat.md](./docs/subsystems/combat.md)
...
- `parties` — the party operational core's durable roster store (`Party` Document, one row per durable party, unique index on `name`, indexed on `memberIds` + `durable`; the `Group`/`channels` precedent — a party owns its membership, ad-hoc parties live only in the `PartyRegistry` in-memory map and are never written here). Behind `PartyApi`/`PartyLogic` over the `PartyRegistry` singleton; the `party:<id>` `GroupProvider` reads it; see [party.md](./docs/subsystems/party.md)
