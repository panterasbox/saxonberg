...
- **Command YAML views**: engine verbs in `mud/cmd/<category>/`,
  ...
  `wind`/`adjust`/`switch`/`fold`/`unfold`; `lock`/`unlock` stay under
  `boundary`).
...
- **Backing-class path mirrors template path** (convention, not
  ...
  **The one deliberate exception is the `*Logic` singletons**: the
  ...
  logic — `obj/api/OfficeLogic.ts` registers at `/obj/api/office` (same
  directory, different leaf; module-id `/obj/api/OfficeLogic` ≠ template
  `/obj/api/office`). New code should follow the mirror unless it's an
  Api/logic singleton.
...
- `office_holders` — the sparse government-office handoff store (`OfficeHolder` Document, one row **only per handed-off office**, `{officeKey, holderId}`, the `groups` precedent with a single scalar holder; absence of a row = the founder default holds the seat). Empty at founding; `assign` upserts, `vacate` deletes. Behind `OfficeApi`/`OfficeLogic` over the `OfficeRegistry` singleton; see [governance.md](./docs/subsystems/governance.md)
...
- `combat_attribution_events` — the combat **blame ledger** (`CombatAttributionEvent`, one append-only row per attribution act `opened`|`violated`|`death`, indexed on `victim` + `sessionId`; the `RenownEvent`/`AuthoringEvent` shape — durable `templatePath` ids, dumb store / smart consumer). Culpability is **derived on read** (`CombatAttributionEvent.deriveBlame` — earliest death row; `crime` = a sentient killed under non-consented lethal terms), never a stamped stat. Written fire-and-forget by the gated `CombatLogic`, read via `CombatApi.blameFor`/`attributionFor`; see [combat.md](./docs/subsystems/combat.md)
