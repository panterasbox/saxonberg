...
- Subsystem references in `docs/subsystems/`. Each doc is the source
  ...
  - [fasttravel.md](./docs/subsystems/fasttravel.md) — the TPA teleport network, now a **utility that runs on mana**: the `tpa` capability pack (the kernel keeps only the physics), FastTravelMixin nodes, the travel credential, the four-way `teleport` fork (free-in-your-extent · the ride · the board-for-everyone · the anchored spell), the three supplies + the arming floor + the amber band, the derived mana rate, the self-governing Authority
