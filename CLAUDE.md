...
- Subsystem references in `docs/subsystems/`. Each doc is the source
  ...
  - [cockpit-layouts.md](./docs/subsystems/cockpit-layouts.md) — the client arrangement layer (client owns ZERO command semantics): two server-authoritative `clientState` axes — **`cockpit.layout`** (the `layout` verb + `LAYOUT_NAMES` vocabulary + the client `LAYOUT_REGISTRY`; `mainView`/`recognizeForumNavigation` deleted; world/forum/livestream-viewer/streamer (a control-deck scaffold)/builder; no auto-switch) and **`cockpit.inputModes`** (per-bar `{barId→prefix}` map — **transient**/session-scoped/never-persisted; the `mode` verb [+ explicit `--bar <id>` target] + the load-bearing pure `CommandApi.applyInputMode` interpreter prepend hooked in `executeCommand` behind `barId !== undefined && interactive && !forced`; absent barId = un-moded); the platform-agnostic `StreamSource` embed (`livestream.broadcastSources` AppSetting → `StreamSourceApi.current()` → welcome snapshot + `StreamSourcesChanged` live push; Twitch wired/YouTube shape-only; chat-topic allowlist terminal split); the CMS re-homed into `builder` (WS live, `?surface=cms` takeover retired); the client display-only inline-prefix bar + **ghost command line** (preview/copy; affordance clicks un-moded) + the **summoned-pane tier** (`SettingsPane` first consumer, notifications degrade-gracefully)
  ...
  - [twitch-relay.md](./docs/subsystems/twitch-relay.md) — **[superseded → streaming.md]** the Twitch-specific transport detail (EventSub session + Helix + incremental-scope reauth + the three-case identity bridge). The two-way **Twitch chat** bridge (broadcast-patronage Phases 2+6): a **separate surface** (own `twitch` verb + `world.twitch.message` topic), NOT a chat `Channel` — Twitch chat is an OOC/meta layer. **Player-initiated, memory-resident, lazy**: `twitch tune <handle>` resolves login→broadcasterId via Helix `/users` once, held in-memory on the `TwitchRelay` singleton (no Mongo registry, no persistent tuning), dropped on logout, re-resolved on re-tune. Backend `TwitchClient` (EventSub session + Helix + token refresh, event-type-parameterized for the deferred payment-intake reuse) + `TwitchRelayReader` (presence-gated subscription set, debounced, `PlayerLoggedOut`-driven unsubscribe); inbound = backend→mud down-call (`dispatchInbound`), outbound/resolve/subscribe = the **Api→backend bridge** (`TwitchLogic` imports the reader directly — the `ConnectionLogic`→`ConnectionManager` pattern; no DI port, no presence event). The **incremental-scope reauth** flow (`twitch-reauth` Passport strategy + `/auth/twitch/reauth`, `force_verify`, write-back to the existing `TwitchProfile`, `hasScope`) acquiring the reader account's `user:read:chat` (env `TWITCH_READER_USER_ID`) + each poster's `user:write:chat` (reject-and-point). The **three-case `RelaySpeaker`** identity bridge (in-game / external / external-linked) — honest-to-origin handle default + linked persona on hover; send-then-mirror, echo tag-and-suppress, token-bucket throttle; subscription-gated not implant-gated; `twitchTemplate` client pane. Video-overlay tuning + YouTube deferred
