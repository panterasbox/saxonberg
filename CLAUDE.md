...
- Subsystem references in `docs/subsystems/`:
  ...
  - [senses.md](./docs/subsystems/senses.md) — multi-sense authoring
    ...
    `Mml.stripBySense` + `Mml.collectSenseChannels` walk the parsed
    tree (drop out-of-filter regions / collect surviving channels);
    `Mml.augment` static (the bare `augmentMarkup` export was
    retired); `BodyPlan.getModalities()` +
    ...
    Perception topics are channel-named, not verb-named:
    `world.perception.{vision,hearing,smell,touch,taste,gestalt}` —
    derived from the FRAME'S body content (Mml.collectSenseChannels
    on the filtered prose), so `sense`-on-vision-only-room fires
    the same topic as `look` (both = `world.perception.vision`);
    `look` and the 17 other "user-reads-output" controllers
    (instruments, find, focus, settings, var) all emit at
    `world.perception.vision`. `Mobile.autoLookOnArrival` renamed
