...
- **Command YAML views**: engine verbs in `mud/cmd/<category>/`,
  ...
  `tally`/`wind`/`adjust` bundle is the exemplar — see
  [command-spec.md](./docs/subsystems/command-spec.md)). A spec's
  `controller:` value is a **path**, resolved by one rule (no domain
  special-case): absolute (`/obj/command/<cat>/<Name>Controller`) or
  relative-to-the-spec (`../command/<Name>Controller`). Categories: perception,
