...
- Subsystem references in `docs/subsystems/`. Each doc is the source
  ...
  - [studio.md](./docs/subsystems/studio.md) — the **mixin-aware composition surface** (the model-aware half of the CMS; sibling of cms.md's file/tree surface): the gated `StudioApi`/`StudioLogic` pair at `/obj/api/studio` (`describeClass` = effective mixin set × the **source-scanned `@authorable`/`@runtimeState` classification** × TypeDoc/runtime type shape × effective value; `listBlueprints`/`getBlueprint`/`publishBlueprint`; `scaffoldClass`/`commitClass`/`listMixins`); the **field-schema derivation** (inline `@authorable`/`@runtimeState` tags on every registry mixin field + the `authorable-coverage` guard, since TypeDoc can't reflect mixin instance fields — the projector's `authorable-fields.json` is sparse enrichment only); the **blueprint catalogue** (`Blueprint` reference-data Document keyed on durable `blueprintId` + structural `signature`, the boot-warmed `BlueprintCatalogue`, the two-layer derived-skeleton + curated-`blueprints.yaml` `BlueprintSeeder`); the **trust model** (three creation acts — instantiate / publish-blueprint / new-code; authoring-free-publish-gated; **save-as-disposition** committed/denied/[reserved]proposed; wizard-lockdown bridge + warning banner; class-then-template ordering; no runtime dynamic compose); the client Studio form + widget registry + **data-integrity overlay** (byte-identical round-trip) + class picker + mixin palette + reference-picker; forward seams (non-wizard `/home` draft `_persistDraft` no-op, the git + propose-review workflow, alias auto-approve, per-type/zone editors) all deferred
